...
public class JmsConfig {
    ...
    @Bean
    EmbeddedActiveMQ getMqServer() {


        if ( serverUrl.startsWith( "vm:" ) || serverUrl.contains( "localhost" ) ) {

            logger.info( CsapApplication.arrowMessage( "Building embedded broker: {}" ), serverUrl );
            EmbeddedActiveMQ server = new EmbeddedActiveMQ( );
            var config = new ConfigurationImpl( );
            try {
//                config.addAcceptorConfiguration( "in-vm", "vm://0" );
                config.addAcceptorConfiguration( "tcp", serverUrl );
                config.setSecurityEnabled( false );
                config.setPersistenceEnabled( false );
                server.setConfiguration( config );
                server.start( );
            } catch ( Exception e ) {
                logger.warn( "Failed to create embeded server: {}", CSAP.buildCsapStack( e ) );
            }

            logger.info( CsapApplication.arrowMessage( "Started embedded broker" ) );
            return server;
        }

        logger.info( CsapApplication.arrowMessage( "Skipping embedded broker: {}" ), serverUrl );
    ...
    public ConnectionFactory getJmsListenersConnectionFactory( ) {
        ...
        logger.info( CsapApplication.arrowMessage( "Building factory now" ) ) ;
        ...
        try {
            ...
            logger.warn( "Failed to set brokerUrl: {}", CSAP.buildCsapStack( e ) );
        }

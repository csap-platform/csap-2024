...
public class KubernetesApiTests extends CsapThinNoProfile {
	...
	public void beforeAll ( )
		...
		try {
			...
			logger.info("building kubernetes api proxys") ;
			...
			logger.info( "Failed to build connection {}", CSAP.buildCsapStack( e )) ;
		}

		logger.info("completed building kubernetes api proxys") ;
	...
	class Statefull {
		...
		public V1PodSpec buildPodSpec ( String containerName ) {
			...
			// nodeSelector.put( "kubernetes.io/hostname", "csap-dev04.csap.org" ) ;
...
//					.contains( "Delete Succeeded - known issue" ) ;
	...
	class Stateless {
		...
		public void list_nodes ( )
			...
			try {
				V1Node dev4 = apiV1.readNodeStatus( "csap-dev04.csap.org", null ) ;
				// s.getStatus().
				// dev4.getStatus().al

				logger.info( "dev4.getStatus : {} ", gsonWithExclusions.toJson( dev4.getStatus( ) ) ) ;
			} catch ( Exception e ) {
				logger.warn( "Failed to get status: {}", CSAP.buildCsapStack( e ) );
			}
...
//									"/api/v1/namespaces/kube-system/pods?limit=1",
		...
		public void field_selectors ( )
			...
			var podFieldSelector = "spec.nodeName=csap-dev04.csap.org" ;
			...
			// var eventFieldSelector = "source.host=csap-dev04.csap.org" ;

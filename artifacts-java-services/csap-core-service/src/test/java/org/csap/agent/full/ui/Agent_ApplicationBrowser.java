...
//				environmentHeader, environmentHeader.asNormalizedText( ),  environmentHeader.asXml( ) ) ;
	...
	public void fileMonitor ( @Autowired MockMvc mockMvc )
		...
		ResultActions resultActions = mockMvc.perform(
				...
				get( CsapConstants.FILE_URL + CsapFileMonitor.FILE_MONITOR )
	...
	@Test
	public void verify_host_config ( )
			throws Exception {

		logger.info( CsapApplication.testHeader( ) );

		var hostConfigReport = appBrowser.hostConfiguration( null,null,null,null );

		logger.debug( "hostConfigReport: {} ", CSAP.jsonPrint( hostConfigReport ) );

		CSAP.jsonStream( hostConfigReport.path( "commandRunnerFiles" ) ).forEach( commandReport -> {
			var failedToLoad =  commandReport.path( "error" ).asBoolean() ;
			if ( failedToLoad ) {
				logger.warn( "Failed to load: {}" , CSAP.jsonPrint( commandReport ))  ;
			}
			assertThat( failedToLoad ).isFalse();
		});
	}
...
//				.contains( "localhost: k8-crashed-service-1: Disk Space: 998Mb, Alert threshold: 150Mb" ) ;
	...
	public void verify_namespace_monitors ( @Autowired MockMvc mockMvc )
		...
		var systemNsReport = appBrowser.service_instances( true, systemNamespaceMonitor.getName( ) ) ;
	...
	public void verify_service_instances_for_multiple_k8_containers_on_master ( )
		...
		var csapTestInstanceReport = appBrowser.service_instances( true, k8TestInstance.getName( ) ) ;
	...
	public void verify_service_instances_for_multiple_k8_containers_on_worker ( )
		...
		var serviceInstances = appBrowser.service_instances( true, k8TestService.getName( ) ) ;
	...
	public void verify_service_instances_for_agent ( )
		...
		var serviceInstances = appBrowser.service_instances( true, agentInstance.getName( ) ) ;
	...
	public void verify_service_instances_for_docker_service ( )
		...
		var serviceInstances = appBrowser.service_instances( true, nginxInstance.getName( ) ) ;
	...
	public void verify_service_instances_for_k8_not_running_on_master ( ) throws Exception {
		...
		var serviceInstances = appBrowser.service_instances( true, k8NotRunningService.getName( ) ) ;

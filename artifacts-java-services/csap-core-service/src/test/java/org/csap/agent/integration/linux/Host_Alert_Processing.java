...
public class Host_Alert_Processing extends CsapThinTests {
	...
	void verify_kill_runaways ( )
		...
		var csapTestService = getApplication( )
				.getServiceInstanceCurrentHost( TEST_SERVICE_NAME ) ;

		// This gets fields that are normalized
		var response_test_data = (ObjectNode) getJsonMapper( )
				.readTree( new File( getClass( )
						.getResource( "host-status-response.json" )
						.toURI( ) ) ) ;

		var serviceData = response_test_data.at( "/" + TEST_HOST_IN_DEFINTION + "/services/"
				+  TEST_SERVICE_NAME  ) ;
		logger.info( "service runtime data from file: {}", CSAP.jsonPrint( serviceData ) ) ;

		var csapTest_loaded = ServiceInstance.buildInstance( getJsonMapper( ), serviceData ) ;
		csapTestService.mergeContainerData( csapTest_loaded.getContainerStatusList( ) ) ;
		logger.info( "testService: {}", csapTestService ) ;
		logger.info( "containers: {}", csapTestService.getContainerStatusList( ) ) ;

		logger.info( "Updated '{}', runtime data: \t active: '{}' \t matcher: '{}' \t path: '{}' \t diskUsage: '{}' ",
				csapTestService.getServiceName_Port( ),
				csapTestService.getDefaultContainer( ).isInactive( ),
				csapTestService.getDiskUsageMatcher( null ),
				csapTestService.getDiskUsagePath( ),
				csapTestService.getDefaultContainer( ).getDiskUsageInMb( ) ) ;

...
class Admin_ApplicationBrowser {
	...
	void beforeAll ( )
		...
		var testHostStatusManager = new HostStatusManager( "agent/runtime-host-k8.json", csapApis ) ;
		...
		assertThat( csapApp.load_junit_definition_start_collectors( false, csapApplicationDefinition ) ).as( "No Errors or warnings" )
				.isFalse( ) ;
...
//				"agent-tab",
	...
	void verify_launchers ( ) {
		...
		logger.info(  CSAP.buildDescription( "launchers", launchers ) ) ;

		assertThat( launchers.size( ) )
				.isEqualTo( 3 ) ;
		
		var launcherWithUrl = launchers.path( 2 ) ;

		assertThat( launcherWithUrl.path( "url" ).isTextual( ) ).isTrue( ) ;
		
		var service = csapApp.findFirstServiceInstanceInLifecycle( "SimpleDocker" ) ;
		
		var serviceLauncher = appBrowser.serviceLauncher( service.getName( ), 2 ) ;
		

		logger.info(  CSAP.buildDescription( "serviceLauncher", serviceLauncher ) ) ;
		


		assertThat( serviceLauncher.path( "location" ).asText( ) ).isEqualTo( "https://junit-demo.com/demo/admin/folder" ) ;
	...
	public void verify_service_summary_for_k8_deploy ( )
		...
		var testService = csapApp.getServiceInstanceCurrentHost( "csap-test-k8s-service" ) ;
		...
		assertThat( k8CrashServiceAlertReport.toString( ) )
				...
				.contains( "150" )
				.contains( "1000" )
				.contains( ServiceAlertsEnum.TYPE_PROCESS_CRASH_OR_KILL ) ;
...
//						"localhost: k8-crashed-service-1: no active OS process found, and no user stop was issued. Probable crash or runaway resource kill: review logs." ) ;
	...
	public void verify_service_instances_for_agent ( )
		...
		var servicesOnHost_text = csapApp.getServicesOnHost( ).stream( )
				.map( ServiceInstance::toSummaryString )
				.collect( Collectors.joining( "\n" ) ) ;

		logger.info( "Host: {}, servicesOnHost: {}", csapApp.getCsapHostName( ), servicesOnHost_text ) ;
		...
		var agentInstances = csapApp.serviceNameToAllInstances( )
				.get( agentInstance.getName( ) ) ;
		assertThat( agentInstances.size( ) )
				.as( "agent instance count" )
				.isEqualTo( 9 ) ;

		var instanceReport = appBrowser.service_instances( true, agentInstance.getName( ) ) ;
	...
	public void verify_service_instances_for_multiple_k8_containers ( )
		...
		var instanceReport = appBrowser.service_instances( true, csapTestService.getName( ) ) ;
...
//				inactiveService.getName( ), false, "default", null, null ) ;
		...
		var status_with_all_stopped = appBrowser.service_instances( true, inactiveService.getName( ) ) ;
	...
	public void verify_service_summary_when_k8_worker_running ( )
		...
		logger.info( "loaded: {}",  inactiveService.toSummaryString( ) ) ;

		HostStatusManager.testHostNameFilters = Arrays.asList( 
				"filter-for-master2-host",
...
//				.doesNotContain( "master2-host: inactive-k8s-service" + ServiceAlertsEnum.KUBERNETES_PROCESS_CHECKS ) ;
	...
	public void verify_service_instances_when_k8_worker_running ( )
		...
		var status_with_worker_started = appBrowser.service_instances( true, inactiveService.getName( ) ) ;

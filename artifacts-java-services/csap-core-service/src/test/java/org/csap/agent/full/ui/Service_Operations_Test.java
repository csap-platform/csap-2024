...
import org.csap.agent.begin.Agent_context_loaded;
...
public class Service_Operations_Test {
	...
	void beforeAll ( )
		...
		assertThat( csapApp.load_junit_definition_start_collectors( false,
				Agent_context_loaded.SIMPLE_TEST_DEFINITION ) ).as(
	...
	public void activity_count_from_event_service ( )
		...
		var csapApplicationDefinition = new File(
		...
		assertThat( csapApp.load_junit_definition_start_collectors( false, csapApplicationDefinition ) ).as(
				"No Errors or warnings" )
				.isTrue( ) ;
		...
		var resultActions = mockMvc.perform( get( CsapConstants.SERVICE_URL + ServiceRequests.ENV_REPORT_URL )
				.accept( MediaType.APPLICATION_JSON ) ) ;

		// But you could do full parsing of the Json result if needed
		var envReportText = resultActions
				.andExpect( status( ).isOk( ) )
				.andReturn( )
				.getResponse( )
				.getContentAsString( ) ;

		var envReport = (ObjectNode) jacksonMapper.readTree( envReportText ) ;

		logger.info( "envReport: {}", jacksonMapper.writerWithDefaultPrettyPrinter( )
				.writeValueAsString( envReport ) ) ;

		assertThat( envReport.path( "eventsIn24hours" ).asInt( ) )
	...
	public void get_service_report_from_analytics_service ( )
		...
		var csapApplicationDefinition = new File(
				getClass( ).getResource( Agent_context_loaded.TEST_DEFINITIONS
						+ "sub-project-localhost/clusterConfigMultiple.json" )
						.getPath( ) ) ;
		var message = "Loading a working definition with multiple supporting services: "
				+ csapApplicationDefinition.getAbsolutePath( ) ;
		logger.info( message ) ;
		...
		logger.info(CsapApplication.testHeader( "Hitting /report " )) ;

		var resultActions = mockMvc.perform( post( CsapConstants.SERVICE_URL + "/report" )
...
//	}
	...
	public void build_service_using_git_no_auth_required ( )
		...
		var svcName = "simple-service_8241" ;
		...
		var scmCommand = "-Dmaven.test.skip=true clean package deploy" ;
		var javaOpts = "-Xms128M -Xmx128M -XX:MaxPermSize=128m" ;
		var scmUserid = "" ; // general service - use empty userid to
								// bypass
		...
		var scmBranch = VersionControl.GIT_NO_BRANCH ;
		...
		boolean sourceCheckoutOk = serviceManager.deployService(
				...
				scmUserid, encryptor.encrypt( scmPass ), repoPass,

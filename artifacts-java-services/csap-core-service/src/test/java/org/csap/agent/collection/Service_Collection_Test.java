...
import org.csap.agent.stats.service.JmxCommonCollector;
...
public class Service_Collection_Test extends CsapThinTests {
	...
	void beforeAll ( )
		...
		logger.warn("remote collection disabled, only classpath collections will be done unless enabled") ;
		getApplication().configuration().setDisableRemoteCollection( true );

		assertThat(
				getApplication( ).load_junit_definition_start_collectors( false, collectionDefinition ) )
	...
	boolean isSetupOk ( ) {
		...
		logger.info( CSAP.buildDescription( "Set up variables",
				"testService", testService + " --- " + getApplication( ).getServiceInstanceCurrentHost( testService )
						.toSummaryString( ),
				"testDbHost", testDbHost,
				"testAdminHost1", testAdminHost1,
				"" ) ) ;
...
//					.isGreaterThan( 99999 ) ;
			...
			logger.info( "agent status: {}",
					CSAP.jsonPrint( runtimeReport.at( HostKeys.servicesJsonPath( CsapConstants.AGENT_NAME ) ) ) ) ;

			var agentThreadPath = HostKeys.serviceMetricJsonPath( CsapConstants.AGENT_NAME, 0,
	...
	@Nested
	@Tag ( "containers" ) // add -DexcludeTags="none" to run manually
	@TestInstance ( TestInstance.Lifecycle.PER_CLASS )
	class JMX_Collection {

		@Test
		void verify_java_collection_for_service1 ( )
			throws Exception {
			// ref build.gradle : To test host behind ssh networks, add the following
			// -DexcludeTags="none"
			// when tags = none, -DsocksProxyHost=localhost -DsocksProxyPort=7197 will be added

			// Must be added when testing over ssl:
			// ssh tunnel: ssh -f -N -L 12345:YOURHOST:12345 -D 14000 YOURHOST
			// eg. ssh tunnel: ssh -f -N -L 12345:my-core-host2.some.host.pattern:12345 -D 14000 my-core-host2.some.host.pattern
			
			
			// verify ssh: jconsole -J-DsocksProxyHost=localhost -J-DsocksProxyPort=14000
			// service:jmx:rmi:///jndi/rmi://localhost:12345/jmxrmi
			// junit arg: -DsocksProxyHost=localhost -DsocksProxyPort=14000

			logger.info( CsapApplication.testHeader(  ) ) ;

			var targetService = getApplication( ).getServiceInstanceCurrentHost( "multijvm1" ) ;
			...
			var javaGraphReportWithCustomAdded = performJavaCommonAndCustomCollection( services, testDbHost ) ;
			...
			assertThat( javaReportData.path("jvmThreadCount_service1").path( 0 ).asInt( ) )
					.isGreaterThan( 900 ) ;
			assertThat( javaReportData.path("jvmClassLoaded_service1").path( 0 ).asInt( ) )
					.isGreaterThan( 900 ) ;
...
//			CSAP.setLogToDebug( ServiceCollector.class.getName( ) ) ;
			...
			CSAP.setLogToDebug( JmxCommonCollector.class.getName( ) ) ;
...
//			CSAP.setLogToDebug( HttpCollector.class.getName( ) ) ;
			...
			for ( String serviceName : services ) {
				...
				if ( serviceName.equals( "multijvm1" ) ) {

					serviceCollector.testHttpCollection( 10000, testDbHost ) ;
			...
			var standardJavaData = serviceCollector.buildCollectionReport( false, services, 999, 0 ) ;
			...
			if ( isCustom ) {
				...
				for ( String serviceName : services ) {
					...
					var customJavaData = serviceCollector.buildCollectionReport( false, customServices, 999, 0,
	...
	class Http_Collection {
		...
		@Tag ( "containers" )
		@Test
		void verify_http_collection_for_mysql ( ) {

			logger.info( CsapApplication.testHeader( ) ) ;

			var mysqlService = getApplication( ).getServiceInstanceCurrentHost( "mysql" ) ;
			logger.info( "service: {}", mysqlService ) ;

			String[] services = {
					mysqlService.getName( )
			} ;

			//
			//
			//
			var collectionUrl = mysqlService.resolveRuntimeVariables(
					mysqlService.getHttpCollectionSettings( ).path( ModelJson.httpCollectionUrl.jpath( ) )
							.asText( ) ) ;
			logger.info( "mysqlService collection url: {}", collectionUrl ) ;
			...
			// run reports
			//
			var applicationCollectionReport = serviceCollector.buildCollectionReport(
					false, services, 999, 0,
					"CustomCollection" ) ;

			var dataReport = applicationCollectionReport.path( "data" ) ;
			logger.info( "{} Application Data Collected {}", mysqlService.getName( ), CSAP.jsonPrint( dataReport ) ) ;

			assertThat(
					dataReport.path( "bytesReceived" ).path( 0 ).asDouble( ) )
							.isGreaterThan( 1 ) ;
...
//		@Tag ( "containers" )
		@Test
		void verify_http_collection_for_service1 ( )
			throws Exception {

			logger.info( CsapApplication.testHeader( ) ) ;

			var otsService = getApplication( ).getServiceInstanceCurrentHost( "multijvm1" ) ;
			logger.info( "service: {}", otsService ) ;
			...
			//
			//
			var collectionUrl = otsService.resolveRuntimeVariables(
					otsService.getHttpCollectionSettings( ).path( ModelJson.httpCollectionUrl.jpath( ) )
							.asText( ) ) ;
			logger.info( "ots collection url: {}", collectionUrl ) ;
...
//			CSAP.setLogToDebug( HttpCollector.class.getName( ) ) ;
			...
			var dataReport = applicationCollectionReport.path( "data" ) ;
			logger.info( "{} Application Data Collected {}", otsService.getName( ), CSAP.jsonPrint( dataReport ) ) ;

			assertThat(
					dataReport.path( "getAttributeRate" ).path( 0 ).asDouble( ) )
							.isGreaterThan( 1 ) ;
			assertThat(
					dataReport.path( "getAttributeCount" ).path( 0 ).asLong( ) )
							.isEqualTo( 754321 ) ;
		...
		@Test
		void verify_http_collection_for_fio ( )
				throws Exception {

			logger.info( CsapApplication.testHeader( ) ) ;

			var fio = getApplication( ).getServiceInstanceCurrentHost( "fio" ) ;
			logger.info( "service: {}", fio ) ;
			...
			//
			//
			var collectionUrl = fio.resolveRuntimeVariables(
					fio.getHttpCollectionSettings( ).path( ModelJson.httpCollectionUrl.jpath( ) )
							.asText( ) ) ;
			logger.info( "fio collection url: {}", collectionUrl ) ;
			...
			var dataReport = applicationCollectionReport.path( "data" ) ;
			logger.info( "{} Application Data Collected {}", fio.getName( ), CSAP.jsonPrint( dataReport ) ) ;

			assertThat(
					dataReport.path( "threads" ).path( 0 ).asInt( ) )
					.isEqualTo( 2 ) ;

			// Double - but collected as long
			assertThat(
					dataReport.path( "readBwPerSecond" ).path( 0 ).asLong( ) )
					.isEqualTo( 190745 ) ;
			assertThat(
					dataReport.path( "readMeanNs" ).path( 0 ).asLong( ) )
					.isEqualTo( 20241231 ) ;


		}
		@Test
		void verify_http_collection_for_sysbench_mysql ( )
				throws Exception {

			logger.info( CsapApplication.testHeader( ) ) ;

			var sysbench = getApplication( ).getServiceInstanceCurrentHost( "sysbench" ) ;
			logger.info( "service: {}", sysbench ) ;
			...
			//
			//
			var collectionUrl = sysbench.resolveRuntimeVariables(
					sysbench.getHttpCollectionSettings( ).path( ModelJson.httpCollectionUrl.jpath( ) )
							.asText( ) ) ;
			logger.info( "sysbench collection url: {}", collectionUrl ) ;
...
//			CSAP.setLogToDebug( HttpCollector.class.getName( ) ) ;
			...
			var dataReport = applicationCollectionReport.path( "data" ) ;
			logger.info( "{} Application Data Collected {}", sysbench.getName( ), CSAP.jsonPrint( dataReport ) ) ;

			assertThat(
					dataReport.path( "threads" ).path( 0 ).asLong( ) )
					.isEqualTo( 64 ) ;

			// Double - but collected as long
			assertThat(
					dataReport.path( "tps" ).path( 0 ).asLong( ) )
					.isEqualTo( 3232 ) ;

			assertThat(
					dataReport.path( "latencyMs" ).path( 0 ).asLong( ) )
					.isEqualTo( 15 ) ;
...
//		@Tag ( "containers" )
//		@Test
//		void verify_http_collection_for_service1_and_service12 ( )
//			throws Exception {
//
//			logger.info( CsapApplication.testHeader( ) ) ;
//
//			var otsService = getApplication( ).getServiceInstanceCurrentHost( "multijvm1" ) ;
//			var ots2Service = getApplication( ).getServiceInstanceCurrentHost( "multijvm2" ) ;
//			logger.info( "service: {}", otsService ) ;
//
//			String[] services = {
//					otsService.getName( ),
...
//			CSAP.setLogToDebug( ServiceCollector.class.getName( ) ) ;
		...
		public void verify_http_service_upload ( )
			...
			getCsapApis( ).application().configuration().setDisableJmxCollection( true );
			//
			// Perform a collection
			//
			var serviceCollector = new ServiceCollector( getCsapApis( ), 30, true ) ;
			serviceCollector.shutdown( ) ;
//			CSAP.setLogToDebug( ServiceCollector.class.getName( ) ) ;
//			CSAP.setLogToDebug( HttpCollector.class.getName( ) ) ;
//			CSAP.setLogToDebug( JmxCollector.class.getName( ) ) ;

			serviceCollector.testHttpCollection( 500, testDbHost ) ;

			var services = List.of(
					testK8Docker_CollectionId1
			);
			var collectionReport = serviceCollector.buildCollectionReport(
					true, services.toArray( new String[0] ), 999, 0,
					"CustomJmxIsBeingUsed" ) ;
			...
			logger.info( "{}", CsapApplication.smallTestHeader( "invoking serviceCollector.uploadMetrics" ) ) ;

			CSAP.setLogToInfo( ServiceCollector.class.getName( ) ) ;
			var appUploadResult = serviceCollector
			...
			var lastSummaryReport = serviceCollector.get_lastCustomServiceSummary( ) ;
			...
			logger.info( "{}", CsapApplication.smallTestHeader( "building Java report" ) ) ;
			...
			ObjectNode latestPublishReport = serviceCollector.getLatestApplicationMetricsPublished( ) ;
			logger.info( CsapApplication.smallTestHeader( "latestPublishReport: \n {}" ), CsapConstants.jsonPrint(
					latestPublishReport ) ) ;
			...
			logger.info( CsapApplication.smallTestHeader( "Verifying kubernetes service id" ) ) ;
		...
		public void verify_nginx_collection ( )
			...
			// csapApp.shutdown();
			var applicationCollector = new ServiceCollector( getCsapApis( ), 30, false ) ;
			...
			applicationCollector.testHttpCollection( 10000, testDbHost ) ;
			CSAP.setLogToInfo( HttpCollector.class.getName( ) ) ;
			...
			// CsapCore.AGENT_ID
			var httpStatistics = applicationCollector.buildCollectionReport( false, services, 999, 0,
					"CustomJmxIsBeingUsed" ) ;

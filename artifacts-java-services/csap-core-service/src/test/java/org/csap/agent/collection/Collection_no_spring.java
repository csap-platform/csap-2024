...
public class Collection_no_spring extends CsapThinTests {
	...
	void beforeAll ( )
		...
		assertThat( getApplication( ).load_junit_definition_start_collectors( false, Agent_context_loaded.SIMPLE_TEST_DEFINITION ) )
	...
	public void verify_real_time_report ( ) {
		...
		//
		// os shared collection
		//
		var osSharedCollecter = new OsSharedResourcesCollector( getCsapApis( ),
				collectionInterval, false ) ;

		getApplication( ).metricManager( ).getOsSharedResourceCollectorMap( ).put( 
				collectionInterval,
				osSharedCollecter ) ;
		
		getApplication( ).rootProjectEnvSettings( ).getMetricToSecondsMap( ).add(
		...
		serviceCollector.testHttpCollection( 10000, "csap-01.csap.org" ) ;
		...
		var osSharedReport = osSharedCollecter.buildCollectionReport( false, null, 1, 0 ) ;
		logger.info( "osSharedReport disktest: {}", 
				CSAP.jsonPrint( osSharedReport.path( "data" ).path( "diskTest" ) ) ) ;
		...
		assertThat(
				realTimeReport
						...
						.path( JavaCollectionAttributes.heapUsed + "_" + CsapConstants.AGENT_NAME ).asInt( ) )
								.isGreaterThan( 10 ) ;

...
class Default_Application extends CsapBareTest {
	...
	void verify_agent_jobs ( ) throws Exception {
		...
		var agentInstance = service( CsapConstants.AGENT_NAME ) ;

		var jobDefinition = agentInstance.getJobsDefinition( ) ;

		logger.info( "agentInstance: \n\t{} \n\n definition: {}",
				agentInstance,
				CSAP.jsonPrint( jobDefinition ) ) ;

		assertThat( jobDefinition.path( "scripts" ) )
				.hasSize( 8 ) ;

		assertThat( jobDefinition.path( "scripts" ).toString( ) )
				.contains( "CPU Profiling" ) ;

		logger.info( "agentInstance: \n\t{} \n\t jobs: {}",
				agentInstance,
				agentInstance.getJobs( ) ) ;

		var agentRunJobs = agentInstance.getJobs( ) ;

		assertThat( agentRunJobs.size( ) )
				.as( "number of jobs" )
				.isEqualTo( 11 ) ;

		assertThat( agentRunJobs.toString( ) )
				.contains( "CPU Profiling" ) ;

		assertThat( agentRunJobs.get( 0 ).getEnvironmentFilters( ) )
		...
		var allServices = getApplication( ).getActiveProject( ).getServiceInstanceStreamInCurrentLC( )

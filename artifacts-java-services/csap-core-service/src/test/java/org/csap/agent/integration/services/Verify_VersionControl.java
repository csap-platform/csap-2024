...
class Verify_VersionControl extends CsapThinNoProfile {
	...
	void beforeAll ( )
		...
		var serviceInstance = getApplication( ).findFirstServiceInstanceInLifecycle( "simple-service" ) ;
	...
	class PublicRepository {
		...
		public void verify_anonymous_checkout_of_agent_at_github ( )
			...
			var serviceInstance = getApplication( ).findFirstServiceInstanceInLifecycle(
	...
	private void checkOutAndVerifyService ( ServiceInstance serviceInstance , String password )
		...
		var serviceBuildFolder = getApplication( ).getCsapBuildFolder( serviceInstance.getServiceName_Port( ) ) ;

		// old path from github is at root of git serviceInstance.getScmBuildLocation( )
		var servicePomFromGitFile = new File( serviceBuildFolder, "csap-core-service/pom.xml" ) ;
...
//				serviceInstance.getDefaultBranch( ),
		...
		var gitResults = FileUtils.readFileToString( resultOutputFileManager.getOutputFile( ) ) ;
		logger.debug( "gitResults: {}", gitResults ) ;

		logger.info( CsapApplication.testHeader( "Verifying location: {}" ), servicePomFromGitFile ) ;

		assertThat( servicePomFromGitFile )
				.as( "Pom file found" )
				.exists( ).isFile( ) ;

		logger.info( "Found: {}; performing cleanup - deleting: {}", servicePomFromGitFile.getAbsolutePath( ), serviceBuildFolder
				.getAbsolutePath( ) ) ;
		Files.list( serviceBuildFolder.toPath( ) )
				.forEach( System.out::println ) ;

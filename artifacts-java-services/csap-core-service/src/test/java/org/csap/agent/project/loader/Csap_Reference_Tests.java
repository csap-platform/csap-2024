...
import static org.csap.agent.CsapConstants.AGENT_NAME;
...
class Csap_Reference_Tests extends CsapBareTest {
	...
	void beforeAll ( )
		...
		var testHostStatusManager = new HostStatusManager(
				"admin/csap-reference-status.json",
				getCsapApis( ) ) ;
	...
	class Dev_Lifecycle_HA_Project {
		...
		void beforeAll ( )
			...
			assertThat( getApplication( ).load_junit_definition_start_collectors( false, csapReferenceDefinition ) )
	...
	class Dev_Lifecycle {
		...
		void beforeAll ( )
			...
			logger.info( CsapApplication.testHeader( ) ) ;
			var hostNameForEnvResolution = "csap-dev01.csap.org" ;
...
//			
		...
		void verify_legacy_attributes_migrated ( )
			...
			assertThat( getAppEnv( ).getApplicationName( ) ).isEqualTo( "demo-xxx Integration" ) ;
			...
			assertThat( packageEnvSettings.getApplicationName( ) ).isEqualTo( "demo-xxx Integration" ) ;
			...
			assertThat( trends.toString( ) ).contains( CsapConstants.ADMIN_NAME ).contains( AGENT_NAME ) ;
			...
			assertThat( realTimeMeters.toString( ) ).contains( AGENT_NAME ) ;
...
//					getAppEnv( ) ) ;
			...
			assertThat( serviceLimitsForUi.path( JavaCollectionAttributes.jvmThreadCount.name( ) ).asInt( ) ).isEqualTo(
					999 ) ;
		...
		void verify_plus_one_ports ( ) {
			...
			assertThat( service.getUrl( ) ).as( "url with port" ).contains(
					"http://csap-dev01.csap.org:6444/stats" ) ;
		...
		void verify_service_load_from_csap_default_template ( )
			...
			var agentLabels = serviceLabels.path( AGENT_NAME ) ;
		...
		void verify_launch_url ( )
			...
			var firstAgentUrl = getApplication( ).getActiveProject( ).getServiceInstances( AGENT_NAME )
					.findFirst( ).get( ).getUrl( ) ;

			logger.info( "first agent launch url: {}", firstAgentUrl ) ;

			assertThat( firstAgentUrl )
					.isEqualTo( "http://csap-dev01.yourorg.org:8011/app-browser?defaultService=csap-agent" ) ;
			...
			assertThat( firstPostgres.getUrl( ) )
					.isEqualTo(
							"http://csap-dev04.csap.org:8014/api/v1/namespaces/csap-test/services/test-k8s-postgres-service" ) ;
		...
		void verify_active_model_agent_instances ( )
			...
			List<String> hosts = getApplication( ).getActiveProject( ).getServiceInstances( AGENT_NAME )
		...
		@Test
		void verify_modelapi_hosts_for_service( )
				throws Exception {

			logger.info( CsapApplication.testHeader( ) );

			var hosts = modelApi.hostsInCurrentEnvironmentForService( AGENT_NAME ) ;

			logger.info( CSAP.buildDescription( "hosts for " + AGENT_NAME, hosts ) ) ;


			assertThat( CSAP.jsonStream( hosts ).map( JsonNode::asText ).collect( Collectors.toList()) )
					.contains( "csap-dev01.csap.org", "csap-dev20.csap.org" )
					.doesNotContain( "csap-stg01.csap.org" ) ;
		}
		...
		void verify_modelapi_serviceOrder ( ) {
			...
			JsonNode allServicesInOrder = modelApi.serviceNames( null, null, true, true,true,false ) ;
			...
			JsonNode filteredIds = modelApi.serviceNames( null, List.of( "base-os" ), true, true, true,false ) ;
			...
			JsonNode reversed = modelApi.serviceNames( null, List.of( "base-os" ), true, true,true,true ) ;

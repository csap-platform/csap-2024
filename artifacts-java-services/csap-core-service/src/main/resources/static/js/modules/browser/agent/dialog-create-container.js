...
import explorerOperations from "./explorer-operations.js"
...
function create_container_dialog() {
    ...
    function populateContainerDefaults() {
        ...
        if ( portInfo ) {
            for ( let exposedItem in portInfo ) {
                // "80/tcp":
        ...
        if ( volumeInfo ) {
            let volCount = 0;
            for ( let exposedItem in volumeInfo ) {
                let volumeDef = {
    ...
    function getDockerCommandString( commandArray ) {
        ...
        if ( isArray( commandArray ) ) {
            let commandString = JSON.stringify( commandArray, null, "  " );

...
function browser_application() {
    ...
    let _lastTabSelected = null ;
    
    this.initialize = function () {

        _dom.logSection( `initializing browser_application: 1` );
    ...
    function navItemClicked( menuId ) {
        ...
        console.log( `Loading ${ menuId }, _lastTabSelected: ${ _lastTabSelected }`  ) ;

        if ( _lastTabSelected == menuId
            && _lastTabSelected != null ) {
            _preferences.snapNavClosed();
        }
        _lastTabSelected = menuId ;
    ...
    function tabMenuSelected( $menuClicked ) {
        ...
        console.log( `tabMenuSelected: tabId: ${ tabId } menuId: ${ menuId }` );
        if ( tabId == `agent-tab`) {
            let menuName = `${ menuId.substring(0,4) }.` ;
            if ( menuId == "service" ) menuName = "proc." ;
            if ( menuId == "application" ) menuName = "app." ;
            if ( menuId == "processes" ) menuName = "top." ;
            if ( menuId == "memory" ) menuName = "mem." ;
            if ( menuId == "script" ) menuName = "run." ;
            if ( menuId == "command-output" ) menuName = "out." ;
            if ( menuId == "explorer" ) menuName = "ex." ;

            let fullName = `${ menuName }${utils.getPageTitle()}`

            if ( menuId == "system" ) {
                fullName = utils.getHostName() ;

                setTimeout(() => {
                    
                    let tag = utils.getHostTag( fullName ) ;
                    if ( tag ) {
    ...
    function tabSelected( $tabButton ) {
        ...
        console.debug( `viewLoadPromise` );

        _dom.csapDebug( `viewLoadPromise`, viewLoadPromise );

        const viewLoaded = function ( resolvedValue ) {

            // console.debug( `resolved promises`, resolvedValue );
            _dom.csapDebug( `viewLoadPromise`, resolvedValue );

...
function host_explorer() {
        ...
        show: function ( $menuContent, forceHostRefresh, menuPath ) {
            ...
            async function loadTreeAndThenRunHostStatus(  ) {
                ...
                if ( forceHostRefresh || !_explorerTree ) {
                    ...
                    //$templateRepo.append( $( "#tree-controls" ) );

                    explorerOps.hideTreeCommands( true );
                    _explorerTree = $explorerTree.fancytree( buildTreeConfiguration() );

                    //$("#csapDefTree", $explorerTree).append($("#tree-controls") ) ;
    ...
    function tree_activate_node( event, treeActivationObject ) {
        ...
        if ( nodeData.path != "/" ) {
            ...
            $packageInfo.css( "width", Math.round( targetWidth  ) + "px" );
            // $packageInfo.css("max-height", Math.round($( "#linuxTree" ).parent().outerWidth( true ) - 100)) ;
            ...
            switch ( nodeType ) {
                ...
                case hostOps.categories().linuxChef:
                    tree_activate_show_linux_chef_readme( nodeSelected, nodeData );
                    break;
            }
        }
    ...
    function tree_activate_show_linux_chef_readme( nodeSelected, nodeData ) {
        let requestParms = {
        ...
        $.getJSON(
            ...
            .done( function ( readmeReport ) {
                ...
                $info.append( jQuery( '<button/>', {
                    text: 'Browse Files',
                    class: 'csap-button-icon csap-search csap-alt-colors'
                } ) );
                $info.append( "<br/>" );

                $( "#linux-chef-info .description", $packageInfo ).html( $info.html() + readmeReport["response-html"] );

                $( "#linux-chef-info .description code", $packageInfo ).each( function() {
                    let $code = $(this) ;
                    if (  $code.hasClass("") ) {
                ...
                $( "button", $( "#linux-chef-info" ) ).off().click( function () {
                    hide_node_info_panel();

                    let parameterMap = {
                        locationAndClear: readmeReport["path"]
                    };
                    utils.launchFiles( parameterMap );
                } );

                $packageInfo.show();
                $( "#linux-chef-info", $packageInfo ).show();
        ...
        $.getJSON(
            ...
            .done( function ( responseJson ) {
                ...
                $( "button", $( "#linux-service-info" ) ).off().click( function () {
                    ...
                    utils.launchHostLogs( parameterMap );
    ...
    function children_root_load() {
        ...
        children.push( {
            "title": title_with_comment( "CSAP", "", "csapDefTree" ),
            "folder": true,
            "expanded": true,
    ...
    function children_root_load_linux() {
        ...
        children.push( {
            "title": title_with_comment( "Chef",  "loading", "loaded-by-buildCommandButton" ),
            "type": hostOps.categories().linuxChef,
            "csapFilter": true,
            "path": "/",
            "folder": true,
            "lazy": true
    ...
    function children_using_parent_dockerPorts( portArray ) {
        ...
        let $portTable = $( "#dockerPortsTemplate" ).clone();

        let $portBody = $( "tbody", $portTable );
        ...
        for ( let portSetting of portArray ) {
            ...
            if ( !$.isNumeric( portSetting.PublicPort ) ) {
                ...
                // desktop settings
                if ( portUrl.includes( "localhost" )
                    && ! containerUrl.contains("/var/run/docker")) {
                ...
                jQuery( '<a/>', {
                    href: portUrl,
                    class: "csap-link-icon launch-window launch-container-link",
                    target: "_blank",
    ...
    function children_using_array( parentType, event, folderItems ) {
        ...
        for ( let folderItem of folderItems ) {
            ...
            } else if ( nodeType == hostOps.categories().linuxChef ) {
                isFolder = false;
                isLazy = false;
                linuxServiceName = folderItem.label;
    ...
    function getProcessIcon( title, attributes ) {
        ...
        } else if ( title.includes( "kube" ) ) {
            icon = "ft-os-process-k8s";
        } else if ( title.startsWith( "k8-" ) ) {
            icon = "ft-os-process-k8s";
        } else if ( title.includes( "docker" ) ) {
    ...
    function tree_render_item( fancyTreeNode ) {
        ...
        } else if ( nodeType == hostOps.categories().linuxChef && configuration.path == "/" ) {
            fancyTreeNode.setTitle( title_for_items_with_shell( "chef", "Run chef commands", "chef-commands" ).html() )
        }
    ...
    function title_for_items_with_shell(
        ...
        if ( commandTemplateName == "node-describe" ) {
            ...
            if ( commandTemplateName == "docker-volume" ) {
                ...
                //                    .appendTo( $commentDiv ) ;
                buildCommandButton( "docker-volume-report", "Volume Reporter", "csap-graph" )
                    .appendTo( $commentDiv );
            } else if ( commandTemplateName == "chef-commands" ) {

                buildCommandButton( "chef-command", "Chef Logs", "csap-window" )
                    .appendTo( $commentDiv );
            } 
        }
    ...
    function buildCommandButton( commandTemplateName, description, buttonClass = "csap-shell" ) {

        let launchButtonId = commandTemplateName + "TreeButton";

        let $button = jQuery( '<button/>', {
            id: launchButtonId,
            class: `csap-button-icon ${ buttonClass } tree`,
            title: description + " ..."
        } )
        ...
        let launchFunction = function () {
            ...
            if ( description == "Chef Logs") {

                let appParams = { defaultService: "chef" };
                //                        if ( isContainer ) {
                //                            appParams.container = containerName ;
                //                        }
                utils.openAgentWindow( 
                    utils.getHostName(), 
                    "/app-browser#agent-tab,logs", appParams );
    ...
    function title_for_container( configuration ) {
        ...
        //     class: "empty"
        // } )
        let $comment = jQuery( '<span/>', { } )
        $comment.appendTo( $description );

        let rawSize = configuration.attributes[ "allVolumesInMb" ] ;
        let volSize = `${ rawSize } Mb` ;
        try {
            if ( rawSize == -1 ) {
                volSize = "na"
            } else {
                volSize = utils.bytesFriendlyDisplay( configuration.attributes[ "allVolumesInMb" ] * 1024 * 1024 ) 
            }
        } catch { }; 

        jQuery( '<span/>', {
            class: "csap-text-right-clip",
            text: volSize,
            title: `Sum of disk of all mounted volumes: ${ volSize }`
        } )
            // .css( "max-width", "6em" )
            .appendTo( $comment );

        let $containerInfo = jQuery( '<span/>', {
                class: "csap-text-left-clip",
                text: configuration.attributes[ "Image" ],
                title: `${ configuration.attributes[ "Image" ] }`
            } ).appendTo( $comment ) ;
        // $containerInfo
        //     .css( "max-width", "6em" ) ;
        if ( k8sNamespace != "" ) {
            $containerInfo.text( nameSpaceFilterLabel( k8sNamespace ) );
            $containerInfo.attr("title", "namespace-filter")
        }
        //$comment.append( containerInfo );

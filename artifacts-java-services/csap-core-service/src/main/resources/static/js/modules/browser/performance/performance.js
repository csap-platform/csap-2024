...
function performance_views() {
    ...
    function initialize() {
        ...
        $( "input.show-by-env,input.stack-report-lines", $performanceTab ).change( function () {
            showInfraTrends();
        } );
    ...
    function getStatusReport( $displayContainer, forceHostRefresh ) {
        ...
        let theStatusUrl = APP_BROWSER_URL + "/status-report" ;
        _net.httpGet( theStatusUrl, parameters )
            .then( statusReport => {
                processStatusReport( statusReport, $displayContainer );
                // console.log( statusReport ) ;
            } )
            .catch( ( e ) => {
                // log the error, but service may just be restarting, so graceful recovery can happen
                // without user being aware
                _dom.logError ( `Failed retrieving stats ${ theStatusUrl }`, e ) ;
            } );

    }

    function processStatusReport( statusReport, $displayContainer ) {
        ...
        if ( statusReport.cpuCount ) {
            ...
            let loadRounded = statusReport.cpuLoad.toFixed(1);

            let $loadCountIndicator = utils.navigationCount( "#cpu-load-count", loadRounded, statusReport.cpuCount / 2 );
            let $barLoadCountIndicator = utils.navigationCount( "#bar-load-count", loadRounded, statusReport.cpuCount / 2 );

            $loadCountIndicator.attr( "title",
                `  cpu load: ${ loadRounded }`
                + `\ncpu count: ${ statusReport.cpuCount }` );


            $barLoadCountIndicator.parent()
                .css("opacity", "1.0")
                .attr( "title",
                    `  cpu load: ${ loadRounded }`
                    + `\ncpu count: ${ statusReport.cpuCount }` ) ;
    ...
    function addServiceNames( serviceIdMapping ) {
        ...
        if ( ( currentServiceCount - 1 ) !== Object.keys( serviceIdMapping ).length ) {
            ...
            jQuery( '<option/>', {
                text: utils.showDefaultServicesName()
    ...
    function buildAlertsList( statusReport ) {
        ...
        if ( errors.length !== 0 ) {
            ...
            for ( let errorRawMessage of errors ) {
                ...
                if ( firstWordInErrorMessage.includes( ":" ) ) {
                    try {
                        ...
                        let $healthLink = null;
                        if ( targetMessage.includes( healthReportToken ) ) {

...
function deployments_views() {
    ...
    function show( $menuContent, forceHostRefresh, menuPath ) {
        ...
        if ( operation ) {
            ...
            if ( !_statistics ) {
                console.log( `Building service stats` );
                _statistics = new Statistics( utils.findContent( "#services-tab-statistics" ) );
            }

            _statistics.load(
                forceHostRefresh,
                instances.getSelectedService(),
                instances.getSelectedHosts()[ 0 ],
                instances.getAllServices() );
    ...
    function addClusters( servicesReport ) {
        ...
        let processGroup = servicesReport.processGroupClusterReport;
        ...
        for ( let clusterName of clusterNames ) {
            ...
            // if ( processGroup[ clusterName ] ) {
            //     let active = processGroup[ clusterName ][ "active" ];
            //     if ( active ) {
            //         totalRunning = active;
            //     }
            //     let stopped = processGroup[ clusterName ][ "stopped" ];
            //     if ( stopped ) {
    ...
    function addServices( servicesReport ) {
        ...
        for ( let serviceName of serviceNames ) {
            ...
            let imagePath = `${ IMAGES_URL }/${ getServiceImage( servicesType[ serviceName ], serviceName ) }`;
    ...
    function getServiceImage( serviceType, serviceName ) {
        let typeToImage = {
        ...
        let serviceImage = typeToImage[ serviceType ];

        if ( serviceType.includes( "tomcat" ) ) {
            serviceImage = 'tomcatLarge.png';
        }

        if ( serviceName == "multijvm1" ) {
            serviceImage = '32x32/preferences-system-session.png';
        }
        if ( serviceName == "wats" ) {
            serviceImage = '32x32/document-save.png';
        }

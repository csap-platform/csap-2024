...
function projects_loader() {
    ...
    const $createCsapEventButton = $( "header #create-csap-event" );
    ...
    function showCreateEventDialog() {
        ...
        let dialogId = "createEventDialogDialog";
        let $dialog = $( "#create-event-dialog" );

        $( ".graph-hover-container" ).hide();

        let createEventDialog;

        if ( !alertify.createEventDialog ) {

            console.log( "Building: dialogId: " + dialogId );

            $( "button", $dialog ).click( function () {
                createEventDialog.close();
                let id = $( "input#note-id", $dialog ).val();
                let summary = $( "input#note-summary", $dialog ).val();
                let eventDetails = $( "textarea", $dialog ).val();
                //alertify.notify( `create event: ${ eventDetails }` );

                let url = utils.getCsapBrowserUrl() + "/userEvent"

                // _net.httpGetAndWait( url, { eventDetails } )
                //     .then( results => {
                //         console.log( `${ url } results`, results ) ;

                //         if ( results.error ) {
                //             alertify.csapWarning( `failed to create event \n\n ${JSON.stringify( results, "\t", "\n") }`)
                //         } else {
                //             alertify.notify( `event Added: ${ results }` );
                //         }
                //     } )

                _net.httpPostForm( url, { id, summary, eventDetails } )
                    .then( results => {
                        console.log( `${ url } results`, results );
                        ...
                        if ( results.error ) {
                            ...
                            alertify.notify( `event Added: ${ JSON.stringify( results, "\t", "\n" ) }` );
                        }
                    } )
                ...
                //     alertify.alert( `Failed Operation: Contact your administrator ${ e } ` );
                // } )
                // .finally( () => {
            ...
            let configuration = {
                ...
                getWidth: function () {
                    return $dialog.outerWidth( true );
                },
                getHeight: function () {
                    return $dialog.outerHeight( true );
                },
            ...
            alertify.dialog( 'createEventDialog', csapDialogFactory, false, 'alert' );


        }

        createEventDialog = alertify.createEventDialog();
    ...
    function initialize() {
        ...
        $createCsapEventButton.click( showCreateEventDialog );
    ...
    function buildEnvironmentViews( environmentReport ) {
        ...
        let discoverReport = environmentReport.discoveryReport;
        let discoveredCount = 0;
        let showAll = $( "#include-all-appids", utils.findContent( "#preferences-tab-content" ) ).is( ":checked" );
        let testAppId = "csap-performance-app";
        testAppId = null;
        if ( utils.getAppId() === "demo-xxxCsap" ) {
            showAll = true;
        }
        if ( discoverReport ) {
            ...
            let appIds = Object.keys( discoverReport );
            ...
            if ( discoveredCount >= 1 ) {
                ...
                for ( let appId of appIds ) {
                    ...
                    if ( ! discoverReport[ appId ] && ! discoverReport[ appId ][ "loadbalancer-url" ] ) {
                        console.warn( "did not find alternate url" ) ;
                        continue ;
                    }
                    let appIdEnvs = discoverReport[ appId ][ "loadbalancer-url" ];
                    let altEnvToUrls = discoverReport[ appId ][ "alt" ];
                    ...
                    let $envPanel = jQuery( '<div/>', {} ).appendTo( $environmentColummn );
                    ...
                    for ( let environmentName of sortedEnvironments ) {
                        ...
                        if ( addDiscovered ) {
                            ...
                            if ( altEnvToUrls && altEnvToUrls[ environmentName ] ) {

                                jQuery( '<option/>', {
                                    value: altEnvToUrls[ environmentName ],
                                    title: `Alternate url`,
                                    text: `==> alternate`
                                } ).appendTo( $discoveryGroup );

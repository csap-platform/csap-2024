...
function project_browser() {
    ...
    function tree_lazy_load( ftEvent, ftNode ) {
        ...
        for ( let fieldName of sortedFields ) {
            ...
            if ( fieldName == "project" ) {
                ...
                title = buildTitle( fieldName, `${ count } service template(s)` );
                ...
                let count = Object.keys( fieldTree ).length;
                let desc = "environment(s)";
                let importDesc = "";
                if ( parentLocation == "environments." ) {
                    desc = "service group(s)";
            ...
            } else if ( fieldName.toLowerCase().includes( "metrics" ) ) {
                extraClasses = "ft_performance";

            } else if ( fieldName.toLowerCase().includes( "monitor" ) ) {
    ...
    function getServerIconStyle( serviceDefinition ) {
        ...
        if ( server.includes( "tomcat" ) ) {
            style = "tomcatEol";
            if ( server.includes( "tomcat8-5" )
                || server.includes( "tomcat9" ) ) {
                style = "tomcatLatest";
            }
        ...
        if ( metaData ) {
            if ( metaData.includes( "isDataStore" ) ) {
                style = "db";
            }
            if ( metaData.includes( "isJms" ) ) {
                style = "jms";
            }
        }

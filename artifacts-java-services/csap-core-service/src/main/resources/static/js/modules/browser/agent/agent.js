...
function agent_views() {
    ...
    const $agentStatistics = $( "#agent-tab-statistics", $agentContent );
    ...
    return {
        ...
        show: function ( $menuContent, forceHostRefresh, menuPath ) {
            ...
            } else if ( menuPath === "statistics" ) {

                utils.loading( `loading statistics module` );
                if ( ! _statistics ) {
                    console.log( `Building agent stats` ) ;
                    _statistics = new Statistics( $agentStatistics ) ;
                }

                _statistics.load( 
                    forceHostRefresh,
                    utils.getSelectedService(),
                    utils.getHostName(),
                     )
    ...
    function initialize() {
        ...
        $( "#copy-top-table", $agentContent ).click( function () {

            let $activeTable =  $( "#processTable", $agentContent ) ;
            utils.copyItemToClipboard( $activeTable );
        } );

        $( "#copy-df-table", $agentContent ).click( function () {

            let $activeTable =  $( "#dfTableDiv table", $agentContent ) ;
            utils.copyItemToClipboard( $activeTable );
        } );
    ...
    function refreshDf() {
        ...
        let foundContainerToFilter = false;
        ...
        const dfResponseHandler = function ( dfJson ) {
            ...
            if ( _autoApplyFilter && foundContainerToFilter ) {
                _autoApplyFilter = false;
                $diskExcludeFilter.val( "pods,overlay,devicemapper,containers,kubernetes.io" );
                //$processFilter.val( "!ns-" );
            }
    ...
    function renderMemoryUi( memoryMetrics ) {
        ...
        let $zingReport = $( "#zing-panel", $agentContent ) ;
        $zingReport.hide() ;
        if ( memoryMetrics?.zing?.run ) {
            $( "pre", $zingReport).text( memoryMetrics?.zing?.plainText ) ;
            $zingReport.show() ;
        } 
    ...
    function updateHostProcessTable( isCsapFilterChecked, processReport ) {
        ...
        $( ".tree-pid-button", $processTable ).off().on( "click", function ( e ) {
            ...
            console.log( `running pid report: ${ pid }` );
            utils.loading( `Running Pid Report` ) ;
    ...
    function addProcessRow(
        ...
        // console.debug( `${ processName } `, containerStats )
        _dom.csapDebug( `${ processName } row data`, containerStats )
        ...
        if ( isProcessRunning ) { // && ! processLabel.serverType
            ...
            let pidForReport = primaryPid;

            // multi matches for os monitors not containers
            if ( containerStats.containerName
                && containerStats.containerName == "default" ) {
                pidForReport = containerPids;
            }
            nameColumn += buildTreeButton( pidForReport );
        ...
        //
        // pss is provided as kb
        //
        $processRow.append(
            buildNumberCell(
                isProcessRunning,
                containerStats.pssMemory * 1024,
                function () {
                    utils.launchScript( {
                        serviceName: processLabel,
                        template: `linux-smem.sh`,
                        pid: primaryPid
                    } );
                },
                true,
        ...
        $processRow.append(
            buildNumberCell(
                isProcessRunning,
                containerStats.virtualMemory * 1024,
                function () {
                    utils.launchScript( {
                        serviceName: processLabel,
                        template: `java-profiling.sh`,
                        pid: primaryPid
                    } );
                },
                true,
        ...
        $processRow.append(
            buildNumberCell(
                ...
                .addClass( "csap-only" )
                .addClass( "ptab-params code-block" )
                .removeClass( "numeric" )
        );
    ...
    function buildNumberCell( isProcessRunning, number, clickFunction, isBytes = true, templateName = "script" ) {
        ...
        if ( clickFunction && isProcessRunning ) {
            jQuery( '<button/>', {
                ...
                title: `launch command runner: ${ templateName }`
            } )
                .click( clickFunction )
                .appendTo( $cell );
        }

...
function host_operations() {
    ...
    let NODE_TYPES = {
        ...
        "os/chef": { icon: "ft-os-chef", iconTooltip: "Chef Recipes" },
    ...
    function commandRunner( $selectOrLink ) {
        ...
        if ( parameters ) {
            ...
            if ( $selectOrLink.attr( "id" ) === $cliRunnerSelect.attr("id") ) {
                    ...
                    , function ( evt, commandToRunAfterUserEditedIt ) {
                        ...
                        $.post( paramUrl )
                            ...
                            .fail( function ( jqXHR, textStatus, errorThrown ) {
                                ...
                                _dialogs.showJsonError(commandToRun, jqXHR) ;
                            } );
                    }
                    , function () {
            ...
            //
            // A few command use plain text responses - so grab as text and then try to parse
            //
            const isText = true ;
            const params = null ;
            _net.httpGet( targetUrl, params, isText )
                .then( commandResultsAsString => {

                    let commandResultsObject = commandResultsAsString ;

                    try {

                        commandResultsObject = JSON.parse( commandResultsAsString )  ;

                    } catch( parseError ) {
                        // assume string
                        commandResultsObject = {
                            "response-sh": commandResultsAsString
                        }

                    }

                    show_command_results_dialog( "Command Output", commandResultsObject );
                } )
                .catch( ( e ) => {
                    console.error( `Failed to run command`, e );
                    _dialogs.showJsonError("Failed to run command", e) ;
                } )
                .finally( () => {
                    utils.loadingComplete() ;
                } );
    ...
    function show_command_results_dialog( operation, commandResults ) {
        ...
        let resultsDialog = getResultsDialog();

        let $closeButton = jQuery( '<button/>', {
            class: "csap-button-icon csap-remove csap-alt-colors",
            text: `Close ${ operation } ` 
        } ).click( function() {
            resultsDialog.close() ;
        })

        $( "#os-results-title" ).empty().append( $closeButton );
        ...
        if ( commandResults[ "response-sh" ] ) {
            ...
            let resultsToShow = commandResults[ "response-yaml" ] ;
            _resultsEditor.setValue( resultsToShow , -1 );

            if ( commandResults.command && commandResults.command.includes( "nmap-output") ) {
                let checkForUrls = function() {

                    let $hostNameDiv = jQuery('<div/>', { } ) ;
                    jQuery('<div/>', {
                        class: "csap-white",
                        html: "Found the following urls in the response - click to open in new window(s)"
                    } ).appendTo( $hostNameDiv ) ;


                    let $launchButtonContainer = jQuery('<div/>', {
                        class: "csap-grey"
                    } )
                    $launchButtonContainer
                        .css("display", "grid")
                        .css("grid-template-columns", "repeat(auto-fit, 20em)")
                        .css("font-size", "12pt")
                        .css("column-gap", "1em") ;
                    $launchButtonContainer.appendTo( $hostNameDiv ) ;


                    const lines = resultsToShow.split(/\r?\n/);
                    let urlsFound = 0 ;
                    ...
                    for ( let line of lines ) {
                        ...
                        if ( line.includes( "/open/tcp") ) {

                            console.log( `matched: ${ line }` )

                            const matches = [...line.matchAll(parenthesisRegEx)].flat();
                            console.log(matches)

                            if ( matches.length > 0) {

                                let theHostMatched = matches[0].replaceAll("(","").replaceAll(")","") ;
                                urlsFound++ ;

                                let theUrlMatched=utils.agentUrl( theHostMatched ) ;
                                if ( line.includes( "8021") ) {
                                    theUrlMatched = theUrlMatched.replaceAll("8011/app-browser#agent-tab,explorer", "8021/csap-admin") ;
                                }

                                let $hostNameLink = jQuery('<a/>', {
                                    target: `_blank`,
                                    class: "csap-link-button launch-window csap-alt-colors",
                                    href: theUrlMatched,
                                    html: `${ theHostMatched }`
                                });
                                $hostNameLink.appendTo( $launchButtonContainer ) ;
                            }
                        }
                    ...
                    if ( urlsFound > 0 ) {
                        ...
                        alertify.alert( $hostNameDiv.html() ) ;
                    }
                }
                setTimeout( checkForUrls, 500 ) ;
            }
    ...
    function resultsViewerResize( dialogWidth, dialogHeight ) {
        ...
        setTimeout( function () {
            ...
            console.log( `resultsViewerResize() launched/resizing editor` );

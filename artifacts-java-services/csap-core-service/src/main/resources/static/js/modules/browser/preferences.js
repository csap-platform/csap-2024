...
function preferences_loader() {
    ...
    const $showGridLines = $( "#show-grid-lines", $prefs );
    const $performGraphSlice = $( "#perform-graph-slice", $prefs );
    ...
    function initialize() {
        ...
        $( "#global-graph-time-zone", $prefs ).change( function () {
            console.log( `updating global-graph-time-zone` );
            updatePreferences();

        } );

        $( "#graph-zero-filter-percent", $prefs ).change( function () {
            console.log( `updating graph-zero-filter-percent` );
            updatePreferences();

        } );

        $( "#graph-label-current", $prefs ).change( function () {
            console.log( `updating graph-label-current` );
            updatePreferences();

        } );

        $( "#graph-label-baseline", $prefs ).change( function () {
            console.log( `updating graph-label-baseline` );
            updatePreferences();
        ...
        //
        // avoid the flash -default in html
        //
        // if ( $( "#auto-hide-navigation" ).prop( "checked" ) ) {
        ...
        $( "#csap-theme", $prefs ).change( function () {
            ...
            if ( utils.isAgent() ) {
                ...
                if ( theme == "auto" ) {
                    ...
                    if ( env.includes( "sand" ) || env.endsWith( "03" ) ) {
                        autoType = "sand";

                    } else if ( env.endsWith( "99" )  ) {
                        autoType = "tree";

                    } else if ( env.endsWith( "wpc" ) || env.endsWith( "installer" ) ) {
                        autoType = "bisque";

                    } else if ( env.includes( "prod" ) || env.includes( "tenant" ) || env.endsWith( "comparison" ) || env.endsWith( "02" ) ) {
                        autoType = "aqua";

                    } else if ( env.includes( "test" ) || env.includes( "qa" ) || env.endsWith( "demo" )  ) {
                        autoType = "tree";

                    } else if ( env.includes( "metal" ) || env.includes( "perf" ) || env.includes( "demo" ) || env.endsWith( "01" ) ) {
                        autoType = "metal";
        ...
        $performGraphSlice.change( function () {
            updatePreferences();
        } );
        ...
        let menuSelectedFunction = function () {
            ...
            if ( targetUrl != "default" ) {
                ...
                if ( isSameWindow || targetUrl.includes( "logout" ) ) {
    ...
    function snapNavClosed() {
        if ( utils.findNavigation().hasClass( "auto-hide" )
            && !$( "#auto-hide-snap" ).is( ":checked" ) ) {

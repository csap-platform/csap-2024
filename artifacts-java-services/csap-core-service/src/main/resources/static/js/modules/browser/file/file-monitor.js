...
function File_Monitor() {
    ...
    let $container, $header, $fileSelection, $csapFileNameFilterEnable, $clearFilterButton,
        $options, $logDefinition, $formatSelection, $scrollCheck, $languageMode,
        $stripAnsi, $legacyDockerSupport, $editorTheme;
    ...
    let _parserLineSeparator, _parserBlankLinesEndingWith, _parserApplyWhen;

    let _throttleEnabled = true;

    let _legacyDockerLogCheck = "" ;
    let _autoDetectLegacyDocker = false;
    ...
    let _aceModeList;
    let _lastFormat = "simple";
    // let _lastTheme = "ace/theme/merbivore_soft"
    let _lastTheme = "ace/theme/kuroir"
    ...
    return {
        ...
        show: function ( $panel, host, service, $theFileSelection ) {
            ...
            $csapFileNameFilterEnable = $( "button#switch-to-combo-select", $header );
            ...
            // older versions of docker duplicated lines
            _autoDetectLegacyDocker = true ;
            $legacyDockerSupport = $( "#legacy-docker-support", $options );
            ...
            if ( utils.areHostSettingsAvailable() ) {
                setTimeout( show, 100 );
            } else {
                setTimeout( function() {
                    if ( utils.areHostSettingsAvailable() ) {
    ...
    function show() {
        ...
        $( 'input.formatter-enabled', $options.parent() ).change( function () {

            console.log( `toggling log wrap2` );
            // $( "#monitor-wrap-text", $options ).trigger( `click` );
            // activeEditor().getSession().setUseWrapMode( $( this ).is( ":checked" ) );

            // if ( $formatSelection.val() == `text` ) {
            if ( $(this).prop( "checked") ) {
                $editorTheme.val( _lastTheme ).trigger( "change" );
                $( "#monitor-wrap-text", $options ).prop( "checked", true );
                activeEditor().getSession().setUseWrapMode( true ) ;
                setTimeout( () => {
                    $editorTheme.val( _lastTheme ).trigger( "change" );
                }, 500 );

                $formatSelection.val( _lastFormat ).trigger( `change` );

            } else {
                _lastFormat = $formatSelection.val();
                _lastTheme = $editorTheme.val();
                $( "#monitor-wrap-text", $options ).prop( "checked", false );
                activeEditor().getSession().setUseWrapMode( false ) ;
                $formatSelection.val( "text" ).trigger( `change` );
            }
        ...
        $( '#grepButton', $container ).click( function () {
            ...
            console.log( `Launching script grep for file: ${ $fileSelection.val() }` );
        ...
        //         $( '#launch-search-script', $container ).trigger( "click" );
        //     }
        // } );

        $( '#launch-search-script', $container ).click( function () {
            ...
            console.log( "Launching search for file2: " + $fileSelection.val() );
            postAndRemove( "_blank", commandScreen, inputMap );
        ...
        if ( logChoice ) {
            ...
            //$( `#logFileSelect option:includes("${ logName }")`, $container ).attr( 'selected', true ) ;
            ...
            $logDefinition.change( function () {
                ...
                for ( let parser of parsers ) {
                    if ( parser.match === $logDefinition.val() ) {
                        ...
                        _parserLineSeparator = parser.lineSeparator;
                        _parserApplyWhen = parser.applyWhen;
                        reloadViewer();
                        break;
                    }
                }
        ...
        $legacyDockerSupport.change( function () {
            reloadViewer();
        } );
        ...
        $editorTheme.change( function () {
            ...
            if ( $formatSelection.val() == "text"
                && !isSelectedFilePathFormatExcluded() ) {
    ...
    function reloadViewer() {
        ...
        $( ".log-load-message", $header ).text( `(re) loading logs` );
        setTimeout( latest_changes_request, 10 );
    ...
    function getMonitorOptions() {
        ...
        options.theme = "ace/theme/kuroir";
    ...
    function latest_changes_request() {
        ...
        //console.log( `latest_changes_request` ) ;

        clearTimeout( checkForChangesTimer );

        if ( $( "#refreshSelect" ).val() <= 0) {
            console.log( `autorefresh cancelled` ) ;
            $( ".log-load-message", $header ).html( 
                `<span title="Use settings to resume tailing" class=status-red>stopped</span>` );
            return ;
        }
        ...
        $.getJSON(
            ...
            .done( function ( fileChangeReport ) {
                processFileChangeReport( fileChangeReport );
            } )

            .fail( function ( jqXHR, textStatus, errorThrown ) {
                _dialogs.showJsonError(  "Retrieving changes for file " + $fileSelection.val(), jqXHR );
            } );
    ...
    function getPodLogs( parameters ) {
        ...
        $.getJSON(
            ...
            .done( function ( podLogs ) {
                ...
                processFileChangeReport( podLogs );
    ...
    function add_content_to_viewer( dockerOrPodReport ) {
        ...
        if ( dockerOrPodReport.containerVersion ) {
            // legacy 17 ce, desktop 20 14
            if ( dockerOrPodReport.containerVersion.startsWith("17")
                && dockerOrPodReport.containerVersion.endsWith("ce")
                && _autoDetectLegacyDocker ) {
                $legacyDockerSupport.prop("checked", true) ;
                _autoDetectLegacyDocker = false ;
            }
        }

        if ( latestText.length > 0
            && $legacyDockerSupport.prop("checked")
            && dockerOrPodReport.since !== -1
            && _legacyDockerLogCheck.endsWith( latestText ) ) {

            //
            // LEGACY DOCKER: Very old docker will ALWAYS output a single line
            //    - if --since and --follow is specified
            // We check for identical lines - and supporess as needed.
            //
            console.log( `latest server logs - suspect legacy docker - ignoring` ) ;
            latestText ="" ;
        } else {
            _legacyDockerLogCheck = latestText ;
        }
        ...
        if ( latestText && latestText.length > 0 ) {
            ...
            if ( latestText.length > ( maxSelectedToDisplay + 200 ) ) {
                ...
                // 
                // console.debug(  );
                _dom.csapDebug( `activePanelId: ${ activePanelId }` );
            ...
            if ( $scrollCheck.is( ':checked' ) ) {
                ...
                _dom.csapDebug( `add_content_to_viewer lineNumber: ${ lineNumber }` );
    ...
    function formatOutput( fileOutput ) {
        ...
        _dom.csapDebug( `formatOutput()  _autoSelectViewFormat ${ _autoSelectViewFormat }` );
        ...
        if ( currentFormat !== "text"
            ...
            _dom.csapDebug( `Parsing using ${ currentFormat }, throttles: `, _parserThrottleWords );
            ...
            if ( _isAutoAssignTheme ) {
                _isAutoAssignTheme = false;
                let theme = "ace/theme/chrome"; // ace/theme/katzenmilch ace/theme/merbivore  ace/theme/tomorrow ace/theme/chrome

                if ( isExternalFormatExclusion() ) {
                    theme = "ace/theme/kuroir";
                    //$languageMode.val( "ace/mode/java" );
                    $languageMode.val( "ace/mode/apache_conf" );
                    activeEditor().getSession().setUseWrapMode( false );
                } else if ( isSelectedFilePathFormatExcluded() ) {
    ...
    function isExternalFormatExclusion() {
        let fileTested = $fileSelection.val();
        return fileTested.includes( `workdayperf/perfdev` )
            || fileTested.includes( `wats` )
            || fileTested.includes( `jmeter` )
            || ( fileTested.includes( `serviceJobs` ));
    // || ( fileTested.includes( `serviceJobs` ) && ! fileTested.includes( `wmu` ));
    }

    function isSelectedFilePathFormatExcluded() {

        let fileTested = $fileSelection.val();
        // let isDeployFound = fileTested.match( ".*/csap-platform/working/[^/]*.log" ) === fileTested ;

        let isDeployFound = fileTested.startsWith( `__working__` ) && fileTested.split( "/" ).length === 2;

        if ( isExternalFormatExclusion() ) {
            isDeployFound = true;
        }

        console.log( `fileTested: ${ fileTested } isDeployFound: ${ isDeployFound } ` );

        return isDeployFound;
    }

    function checkApplicationLogTemplates( selectedFilePath, outputLines ) {


        $( 'input.formatter-enabled', $options.parent() ).prop( "checked", true) ;
        $( "#monitor-wrap-text", $options ).prop( "checked", true );
        if ( isSelectedFilePathFormatExcluded() ) {
            $( 'input.formatter-enabled', $options.parent() ).prop( "checked", false) ;
            $( "#monitor-wrap-text", $options ).prop( "checked", false );
            activeEditor().getSession().setUseWrapMode( $( this ).is( ":checked" ) );
            return false;
        }
        ...
        _parserLineSeparator = null;
        _parserBlankLinesEndingWith = null;
        _parserApplyWhen = null
        ...
        function checkParsers( parserSourcePath ) {
            for ( let parser of parsers ) {
                if ( parserSourcePath.match( parser.match ) ) {
                    ...
                    _parserTrimAnsi = parser.trimAnsi;
                    _parserLineSeparator = parser.lineSeparator;
                    _parserBlankLinesEndingWith = parser.blankLinesEndingWith;
                    _parserApplyWhen = parser.applyWhen;
                    ...
                    if ( parser.disableWrap ) {
                        $( "#monitor-wrap-text", $options ).prop( "checked", false ).trigger( "change" );
                    }
                    return true;
                }
            }
        ...
        if ( outputLines.length > 3 ) {
            ...
            for ( let testLine of outputLines ) {
                ...
                if ( fields.length == 10
                    ...
                    console.debug( `skipping line fields not matched ` );
                }
            }
        }
    ...
    function determineParser( fileOutput ) {
        ...
        if ( selectedFilePath.includes( "-gc" )
            || selectedFilePath.includes( `gc.` ) ) {
            $formatSelection.val( "java gc" );

            $( "#log-throttle-words", $options ).val( "SPS,COMPSTATS" );
            _parserThrottleWords = [ "SPS", "COMPSTATS" ];

            // apache_conf elixir erlang jack less livescript r stylus toml yaml
            $languageMode.val( "ace/mode/yaml" ).trigger( "change" );

            // chrome merbivore_soft
            $editorTheme.val( "ace/theme/merbivore_soft" ).trigger( "change" );
            activeEditor().getSession().setUseWrapMode( false );

            //            } else if ( source.endsWith( "netmet-log4j.log" ) ) {
            //                $formatSelection.val( "columns" ) ;

        } else if ( checkApplicationLogTemplates( selectedFilePath, outputLines ) ) {
    ...
    function formatLine( logLine, currentFormat ) {
        ...
        if ( currentFormat === "simple" ) {
        ...
        } else if ( currentFormat === "java gc" ) {

            let indent = _indent;
            if ( logLine.includes( "gc,start" ) // open jdk, zule
                || logLine.includes( "New GC cycle" ) // zing 8
                || logLine.includes( "Old GC cycle" ) // zing 8
                || logLine.includes( "NewToOld GC cycle" ) // zing 8
                || logLine.includes( "GC New Max" ) // zing 8
            ) {
                indent = "\n\n"
            } else if ( logLine.includes( "[info][gc]" ) ) {
                if ( logLine.includes( "Tiered VM" ) ) {
                    indent = "";
                }
                // indent = "";
                // indent for zing

            } else if ( logLine.includes( "[gc]" ) ) {
                indent = "";

            }

            // else if ( logLine.includes( "gc,phases" ) ) { 
            //     indent += `${ _indent }`
            // } else if ( logLine.includes( "gc,cpu" ) ) {
            //     indent += `${ _indent }`
            // } 
            logLine = indent + logLine;

        } else if ( currentFormat === "kubelet" ) {
            //
            // format 1: typical
            // Mar 22 09:41:42 csap-dev07.csap.org kubelet[650075]: E0322 09:41:42.876820  650075 reflector.go:138] object-"kube-system"/"kubernetes-services-endpoint":
            // format 2: wrapped
            // Mar 31 09:00:36 csap-dev05.csap.org kubelet[8146]: 2021-03-31 09:00:36.536 [INFO][19709] k8s.go 569: Teardown processing complete. ContainerID="xxx"
            // fromat 3:
            // Mar 31 09:00:36 csap-dev05.csap.org kubelet[8146]: time="2021-03-31T09:00:36-04:00" level=info msg="Released host-wide IPAM lock." source="ipam_plugin.go:369"
            //
            // Mar 31 15:35:55 csap-dev07.csap.org systemd[1]: Stopping kubelet: The Kubernetes Node Agent...
            ...
            //logLine = jsYaml.dump( lineAsJson ) ;
            ...
            outputLineSeparator = "\n---";
            if ( _parserLineSeparator ) {
                outputLineSeparator = _parserLineSeparator;
            }

            let fields = utils.stringSplitWithRemainder( logLine, " ", numColumns );
            let isApply = true
            if ( _parserApplyWhen && !logLine.includes( _parserApplyWhen ) ) {
                isApply = false;
            }
                ...
                try {
                    ...
                    for ( let index = 0; index < colsToDisplay.length; index++ ) {
                        ...
                        if ( !isNaN( columnIndex ) ) {
                            ...
                            //
                            //  Final column typically includes message body: apply keywords and throttles
                            //
                    ...
                    if ( _parserApplyWhen ) {
                        let alignFields = utils.stringSplitWithRemainder( composedLine, " : ", 2, false );

                        if ( alignFields.length == 2 ) {
                            composedLine = `${ alignFields[ 0 ].padEnd( 35, " " ) } : ${ alignFields[ 1 ] }`;
                            //console.log( `alignFields: '${ logLine }'`) ;
                        }

                        if ( _parserBlankLinesEndingWith && composedLine.endsWith( _parserBlankLinesEndingWith ) ) {
                            composedLine = "";
                        }
                        if ( composedLine.includes( `status: Error` ) ) {
                            composedLine += "\n#\n##\n### ERROR \n##\n#";
                        }
                    }
    ...
    function processFileChangeReport( fileChangeReport ) {


        if ( fileChangeReport.error ) {

            if ( fileChangeReport.error ) {
                //$htmlViewer.append( '<span class="error">' + changesJson.error + "</span>" );


                let message = fileChangeReport.error;

                message += ", reason: \n\n";

                if ( fileChangeReport.reason ) {
                    message += ", reason: \n\n" + fileChangeReport.reason;
                }
        ...
        add_content_to_viewer( fileChangeReport );
    ...
    function schedule_next_tail( changeReport ) {
        ...
        let statusMessage = `${ $( "#refreshSelect" ).val() }s <span title="refresh interval in seconds" class=status-green></span>`;

        if ( refreshTimer > 0  // do not scroll when  
            && changeReport.newOffset != changeReport.currLength ) {
            refreshTimer = 50; // 50 small delay to allow for disabling

            statusMessage = `${ utils.bytesFriendlyDisplay( changeReport.newOffset ) }`
                + ` of ${ utils.bytesFriendlyDisplay( changeReport.currLength ) } `
                + ` <span title="refresh interval in seconds" class=status-yellow></span>` ;
        ...
        $( ".log-load-message", $header ).html( statusMessage );

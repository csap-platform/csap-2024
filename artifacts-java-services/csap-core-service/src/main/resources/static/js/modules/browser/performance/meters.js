...
function performance_meters() {
    ...
    let _meterHeight = 130; // 130px
    let _meterWidth = 196

    let _meterLargeHeight = 286; // 130px
    let _meterLargeWidth = 410; // 130px
    let _meterLargeCurrentRow = 1;
    ...
    function buildAppLaunchButtons( launchers ) {
        ...
        if ( launchers.length != _lastLaunchCount ) {
            ...
            for ( let launcher of launchers ) {
                ...
                let launcherService = launcher.service ;
                if ( !launcherService ) launcherService="not-defined" ;
                if ( launcherService.includes( "multijvm1" ) 
                        || ( launcher.label && launcher.label.toLowerCase().startsWith( "oms:" ) )  ) {
                    type = 'csap-button-icon csap-wday';

                } else if ( launcherService.includes( "csap-agent" ) 
                        || ( launcher.label && launcher.label.toLowerCase().startsWith( "csap:" ) )  ) {
                    type = 'csap-button-icon csap-trademark';

                } else if ( launcher.label && launcher.label.toLowerCase().startsWith( "perf:" )  ) {
                ...
                } else if ( launcherService.includes( "grafana" )
                    || launcherService.includes( "analytics" )
                    || ( launcher.label && launcher.label.toLowerCase().includes( "adoption" ) ) ) {
                    type = 'csap-button-icon csap-graph';

                } else if ( launcherService.includes( "kubelet" )
                    || launcherService.includes( "mongo" ) ) {
                    type = 'csap-button-icon csap-indent';

                } else if ( launcherService.includes( "prometh" )
                    || launcherService.includes( "mongo" ) ) {
                    type = 'csap-button-icon csap-db';

                } else if ( launcherService.includes( "alert" )
                    || ( launcher.label && launcher.label.toLowerCase().includes( "alert" ) ) ) {
                    type = 'csap-button-icon status-warning';
                }

                let label = launcherService;
                if ( launcher.label ) {
                    label = launcher.label;
                }

                let desc = `Launching: ${ launcherService }`;
                if ( launcher.description ) {
                    desc = launcher.description;
                }

                jQuery( '<button/>', {
                    class: `${ type } csap-alt-colors`,
                    title: desc,
                    text: label,
                    "data-service": launcher.service,
                    "data-path": launcher.path,
                    "data-message": launcher.message,
                    "data-index": launcherIndex++
    ...
    function rebindApplicationLaunchers() {
        ...
        $( "button", $applicationLaunchers ).off().click( function () {
            ...
            let launcherIndex = $button.data( "index" );
            let launcherMessage = $button.data( "message" );
            let isPerf = $button.text().startsWith("PERF");
            console.log( `launching ${ serviceName }` );
            utils.launchService( serviceName, servicePath, launcherIndex, launcherMessage, isPerf );
    ...
    function renderMeter( meterId, meterDefinition, checkBoxFunction ) {
        ...
        if ( meterDefinition.healthMeter && Array.isArray( meterDefinition.healthMeter ) && meterDefinition.healthMeter.length == 3 ) {
            ...
            if ( meterType != null ) {
                ...
                $meterDiv.append( jQuery( '<div/>', { class: meterType } ) );
                $meterDiv.append( $meterValue );
                $meterDiv.css( "height", `${ _meterHeight }px` ).css( "overflow", "hidden" );
                $meterDiv.css( "width", `${ _meterWidth }px` ).css( "overflow", "hidden" );

                console.log( `meterDefinition.label: ${ meterDefinition.label } ` );
                $meters.append( $containerDiv );
                if ( meterDefinition.large ) {
                    addLargeMeterToFirstColumn( $meterDiv )
                }
        ...
        let mWidth = _meterWidth;
        let mHeight = _meterHeight;

        $meterDiv.css( "height", `${ mHeight + 20 }px` ).css( "overflow", "hidden" );
        $meterDiv.css( "width", `${ mWidth }px` ).css( "overflow", "hidden" );
        ...
        if ( !meterDefinition.large ) {
            $meterDiv.css( "height", `${ mHeight }px` ).css( "overflow", "hidden" );
        }
        //$meterDiv.css( "height", `${ _meterHeight }px` ).css( "overflow", "hidden" );
    ...
    function addLargeMeterToFirstColumn( $meterDiv ) {

        $meterDiv.css( "height", `${ _meterLargeHeight }px` ).css( "overflow", "hidden" );
        $meterDiv.css( "width", `${ _meterLargeWidth }px` ).css( "overflow", "hidden" );
        // $meterDiv.parent().css("grid-column", `1 / span 2`) ;
        $meterDiv.parent().addClass( `meter-large` );
        $meterDiv.parent().css( "grid-row", `${ _meterLargeCurrentRow } / span 2` );
        _meterLargeCurrentRow += 2;
    ...
    function addSelectedHistogram( responseJson ) {
        ...
        for ( let j = 0; j < selectedMeter.hostNames.length; j++ ) {
            list.push( {
                'val': selectedMeter.hostValues[ j ],
                'host': utils.getHostWithTag( selectedMeter.hostNames[ j ] ) 
            } );
        }
    ...
    function addToMetricTable( meterReports ) {
        ...
        for ( let meterReport of meterReports ) {
            ...
            for ( let hostIndex = 0; hostIndex < meterReport.hostNames.length; hostIndex++ ) {
                ...
                let hostName = meterReport.hostNames[ hostIndex ];
                ...
                let hostRowId = ( hostName + "Row" );
                $( "#" + hostRowId ).show();

                let metricHostClass = utils.buildValidDomId( meterReport.label + hostName ); // ( meterReport.id + curHost ).replace( /\./g, '_' ) ;
            ...
            for ( let hostNameForData of meterReport.hostNames ) {
                ...
                if ( $( "#" + hostRowId ).length == 0 ) {
                    ...
                    let hostCol = jQuery( '<td/>', {
                        html: utils.getHostWithTag( hostNameForData ),
                    } ).appendTo( hostRow );

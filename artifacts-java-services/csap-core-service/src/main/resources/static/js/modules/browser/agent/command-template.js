...
function command_templates() {
    ...
    let CSAP_ENV_FILE = "/bin/csap-environment.sh" ;
    ...
    const commonScriptHeader = '#!/bin/bash\n\n'
            + '# helper functions \n'
            + 'source ' + CSAP_INSTALL_FOLDER + CSAP_ENV_FILE + ' ; \n\n' ;
...
// |(^\s+)|(\s+$)
    ...
    function templateDialogFactory() {
        ...
        for ( let scriptDefinition of configurationReport.commandRunnerFiles ) {
    ...
    function templateTextLoader( templateContents, templateName ) {
        ...
        // console.log(`updating ${ templateName } with location: '${location}' , ${ templateContents }`);
        console.log(`updating ${ templateName } with location: '${location}'`);

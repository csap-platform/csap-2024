...
function summary_tab() {
    ...
    function runReports() {
        ...
        if ( $( "#lifeSelect" ).val().includes("oms") ) {
            let customParameters = {
                "serviceName": "multijvm1",
                trending: true
            }
            runSummaryForReport( "application/detail", "getAttributeCount", "XO Attributes Read by OTS", customParameters );
            runSummaryForReport( "application/detail", "zingNewPauseTimeDelta", "Zing Pause Time", customParameters );
    ...
    function runSummaryForReport( report, selectedColumn, reportLabel, optionalParameters ) {
        ...
        let displayHeight = $( window ).outerHeight( true ) - $( "header" ).outerHeight( true ) - 300;
        let trendHeight = Math.floor( displayHeight / 3 );

        if ( $("#isLargeGraph").prop("checked") ) {
            trendHeight = Math.floor( displayHeight / 1.2 );
        }
        ...
        if ( optionalParameters ) {
            $.extend( paramObject, optionalParameters ) ;
        }
    ...
    function buildSummaryReportTrend( selectedColumn, meterDiv, report, currentResults ) {
        ...
        const plotOptions = {
            ...
            stackSeries: $( "#stack-report-lines" ).is( ':checked' ),

            seriesDefaults: {
                fill: $( "#stack-report-lines" ).is( ':checked' ),
                fillAndStroke: true,
                fillAlpha: 0.5,
            ...
            highlighter: {
                ...
                tooltipLocation: "ne",
                tooltipOffset: 5,
                formatString: '%s : <span class=summary-value>%s</span><div class="trendSeries"></div><div class="trendOpen">Click to view daily report</div>',

...
function graphs_package() {
    ...
    class ResourceGraph {
        ...
        getSelectedCustomView() {
            ...
            let layoutDetails = this._customViews[ viewSelection ] ;

            if ( layoutDetails == undefined ) {
                return null;
            }
        ...
        getCurrentServiceName() {
            let serviceName = uiSettings.localApplicationService;
            if ( !serviceName ) {
                serviceName = utils.getSelectedService();
            }

            console.log( `currentService 1 for layouts: ${ serviceName }` );
            if ( !serviceName || serviceName == "" ) {
                serviceName = $( "#jmxCustomWhenClassSet" ).text();
            }
            console.log( `currentService 2 for layouts: ${ serviceName }` );
            return serviceName;
        }
        ...
        addCustomViews() {
            ...
            viewLoadedPromise
                .then( customViews => {
                    ...
                    this._customViews = customViews[ this._metricType ];
                    if ( this._metricType == METRICS_APPLICATION ) {

                        let applicationViews = this._customViews ;
                        let serviceLoaded = this.getCurrentServiceName() ;

                        this._customViews = applicationViews[ serviceLoaded ];

                        if ( typeof this._customViews === 'string' ) {
                            console.log( `serviceLoaded: ${ serviceLoaded } points at ${ this._customViews  }` ) ;
                            this._customViews = applicationViews[ this._customViews  ] ;
                            //console.log( this._customViews ) 
                        }
        ...
        releasePlotResources() {
            ...
            _dom.logSection( `Releasing previous plot resources` );
            if ( this._dataManager ) {

                let $containerDescendants = $( ".hostChildren", this.$GRAPH_INSTANCE ).find("*") ; ;
                ( `1 unbinding all graph container items: ${  $containerDescendants.length }`) ;
                $containerDescendants.off();

                for ( let host of this._hostArray ) {
                    let hostPlots = this._dataManager.getHostGraph( host );
                ...
                this._dataManager.removeAllHostGraphs() ;
                $containerDescendants.remove() ;
        ...
        triggerGraphRefresh() {
            ...
            for ( let currentHost of this._hostArray ) {
                ...
                console.log( `triggering metrics draw` ) ;
        ...
        buildHostContainer( host, $GRAPH_INSTANCE ) {
            ...
            $( ".hostName", $container ).html( utils.getHostWithTag( host ) );
            let $helperUrl = utils.getHostHelperUrl( host );
            if ( $helperUrl != null ) {
                $( ".hostName", $container ).append( $helperUrl );
            }
        ...
        getMetrics( numSamples, host ) {
            ...
            if ( $( ".reset-graph-button button", this.$GRAPH_INSTANCE ).length > 0 ) {
                console.log( ` aborting refresh because graph is zoomed` );
                return;
            }
            ...
            if ( uiSettings.localApplicationService ) {
                ...
                //
                // check in app browser if a service has been selected and available
                //
                if ( utils.getSelectedService() 
                    ...
                    //
                    // "default" is when custom services may be selected
                    //
                    if ( utilsServiceName != utils.showDefaultServicesName() ) {
            ...
            // global data loading
            //
            settings.checkForGlobalDate( this.$GRAPH_INSTANCE ) ;
            ...
            let paramObject = {
                ...
                "numSamples": numSamples,   // always get all available - mean/min are double used
                //"numSamples": -1,
                "skipFirstItems": 1,
            ...
            if ( numdays != -1 ) {
                ...
                if ( utils.getActiveEnvironment().includes( "desktop" ) ) {
                    ...
                    paramObject.appId = "csap-performance-app";
                    paramObject.life = "oms-perf-01";
                }
        ...
        getSampleInterval( $GRAPH_INSTANCE, collectionIntervals ) {
            ...
            //     + ']:checked' ).val();
            let $displayIntervals = $( ".sampleIntervals", this.$GRAPH_INSTANCE );
            let sampleTimeSelection = $displayIntervals.val();
            ...
            if ( typeof sampleTimeSelection == 'undefined' ) {
                ...
                if ( numDays > maxAutoShiftDays ) {
                    selectedIndex = this._lastSamplesAvailable.length - 1;
                }
                if ( numDays > maxAutoShiftDays && numDays < ( maxAutoShiftDays*3)
        ...
        intervalsForPerformance( host, type,
            ...
            if ( !packageDefinition ) {
                this.alertOrLog( "Unable to retrieve package information. Verify connectivity with CSAP Event Services" );
            ...
            //     utils.generalErrorHandler( "Performance Metrics", errorThrown );
            // } );
        }
            ...
            if ( utils.isObject( hostData ) ) {
                ...
                if ( Object.keys( hostData ).length > 0 ) {
                    ...
                    if ( Object.keys( hostData ).length > 0 ) {
                        ...
                        if ( utils.isGraphSlicingActive( hostData.timeStamp ) ) {

                            for (let metricArrayName in hostData )  {
                                let metricArray = hostData[ metricArrayName ] ;
                                console.log( `   trimming: ${ metricArrayName }: ${ metricArray.length }`);
                                if ( metricArray.length > ( graphSliceStart + graphSliceAmount) ) {
                                    console.log( `splicing: graphSliceStart ${ graphSliceStart} graphSliceAmount ${ graphSliceAmount }`) ;
                                    // raw data passed in with latest at the top.
                                    metricArray.reverse()
                                    metricArray.splice ( graphSliceStart, graphSliceAmount ) ;
                                    metricArray.reverse()
                                }
                            }
                        }
        ...
        renderGraph( hostGraphData, isSchedule ) {
            ...
            console.log( "metricDataSuccess() hostGraphData: " );
            ...
            let $containerDescendants = $( ".plotContainer", $newGraphContainer ).find("*") ;
            console.log( `2 unbinding all graph container items: ${  $containerDescendants.length }`) ;
            $containerDescendants.off();
            ...
            if ( hostGraphData.data.timeStamp == null ) {
                if ( this._dataManager.getHostCount() <= 1 ) {
                    this.alertOrLog( "No timeStamp in data for selected interval." + host );
                } else {
            ...
            settings.applyPlotFilter( this._currentResourceGraphInstance );
        ...
        scheduleUpdate( host, hostGraphData, $newGraphContainer ) {
            ...
            if ( $refreshInterval.length > 0 ) {
                if ( $refreshInterval.val() == 999 ) {
                    $( ".graphOptions .refresh-window", this.$GRAPH_INSTANCE ).css( "background-color", "#e1d46a" );
            ...
            let intervalText = ( durationMs / 60000 ).toFixed( 1 );
            if ( intervalText == 0.5 ) {
                intervalText = "30s"
            } else {
                intervalText = `<span class="highlight">${ intervalText } minutes</span>`
            }
            $( ".hostInterval", $newGraphContainer ).html( intervalText );

            let numRec = 1;

            if ( !$( ".useHistorical", this.$GRAPH_INSTANCE ).is( ':checked' ) ) {

                this.clearRefreshTimers( host );
        ...
        updateGraphsCheckboxes( graphs ) {
            ...
            for ( let graphName of Object.keys( graphs ) ) {
                ...
                _dom.csapDebug( ` graphLines: `, graphLines );
        ...
        updateServiceCheckboxes( servicesAvailable, servicesRequested ) {
            ...
            _dom.logArrow( `updateServiceCheckboxes: populating service selects: agent service: ${ utils.getSelectedService() }` );
            ...
            for ( let serviceName of servicesToAdd ) {

                let data_service = serviceName
                let label = serviceName;
                // console.log("updateServiceCheckboxes() Adding:" + label) ;
                let foundIndex = serviceName.indexOf( "_" );


                if ( foundIndex > 3 ) {
                    let nameOnly = serviceName.substring( 0, foundIndex );
                    label = nameOnly + "<span>" + serviceName.substring( foundIndex + 1 ) + "</span>";
                    if ( this.isCurrentModePerformancePortal() ) {
                ...
                for ( let serviceInstance of serviceInstances ) {
                    ...
                    if ( serviceName.startsWith( serviceInstance.serviceName ) && serviceInstance.replicaCount ) {
                ...
                for ( let containerCount = 1; containerCount <= numContainers; containerCount++ ) {
                    ...
                    let id = "serviceCheckbox" + serviceName;
                    ...
                    if ( utils.getSelectedService() ) {
                        if ( serviceName.startsWith( utils.getSelectedService() )  ) {
                            $serviceInput.prop( "checked", true );
                        }
                    }

                    if ( $.inArray( data_service, servicesRequested ) != -1 ||
                        $.inArray( serviceName, servicesRequested ) != -1 ) {
        ...
        reDraw() {
            ...
            utils.loading( "Refreshing graphs" );
            ...
            setTimeout( function () {
                ...
                for ( let host of resourceGraph._hostArray ) {

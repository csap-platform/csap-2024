...
#
#   csap-jobs will source csap-environment.sh, which includes all the helper functions
#   source $CSAP_FOLDER/bin/csap-environment.sh
#

function pre_deploy() {
	
	print_separator "Running pre_deploy" ;
	
}

function post_deploy() {
	
	print_separator "Running post_deploy" ;
...
function pre_start() {
	...
	print_separator "Running pre_start" ;
	
} 

function post_start() {
	
	print_separator "Running post_start" ;
	
}

function pre_stop() {
	
	print_separator "Running pre_stop" ;
	
}

function post_stop() {
	
	print_separator "post_stop" ;
...
function samples() {
	...
	if [ ! -d $mount_target ] ; then
		print_separator "Creating nfs mount point: '$mount_target'" ;
	...
	fstab_entry="$mount_source     $mount_target     nfs     vers=3  0 0"
	print_separator "Adding nfs mount to /etc/fstab: '$fstab_entry'"

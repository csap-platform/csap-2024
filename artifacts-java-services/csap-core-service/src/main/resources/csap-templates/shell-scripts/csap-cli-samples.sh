...
print_command "hosts: for specified service" \
	"$(csap model/hosts/currentLifecycle/name/csap-agent --parse)"


print_command "first host: for specified service" \
	"$(csap model/hosts/currentLifecycle/name/csap-agent --jpath /0)"
...
runCommandParameters="echo -e \$(hostname --long);echo hi"
timeOutSeconds=30
print_command "run command on remote agent" \
	"$(run_using_agent_api $(hostname --long) $timeOutSeconds "$runCommandParameters")"
...
#	--params "$(csap_credentials),serviceName=csap-verify-service"
curl -d "$(echo $(csap_credentials) | tr ',' '&')&serviceName=csap-verify-service&fileName=console.log"  http://centos1.csap.org:8011/api/agent/service/log/download

...
public class HealthForAdmin {
	...
	public void service_summary (
									...
									List<String> environmentHostList ,
		...
		logger.debug( "clusterFilter: {}, lifeCycleHostList: {}", clusterFilter, environmentHostList ) ;
		long lastOpMills = csapApis.application( ).getLastOpMillis( ) ;
		service_summary.put( "lastOp", csapApis.application( ).getLastOpMessage( ) ) ;

		var totalHosts = environmentHostList.size( ) ;
		var totalServicesActive = new AtomicInteger( 0 ) ;
		var totalHostsActive = new AtomicInteger( 0 ) ;
		//
		// Deploy manager case
		...
		var clusterSynchronizationMessages = new StringBuilder( ) ;
		...
		for ( var host : environmentHostList ) {

			var hostStatusNode = jsonMapper.createObjectNode( ) ;

			var hostServiceActive = new AtomicInteger( 0 ) ;
			var hostServiceTotal = new AtomicInteger( 0 ) ;

			var agent_status = csapApis.application( ).getHostStatusManager( ).getHostAsJson( host ) ;
	...
	private void find_service_for_summary (
		...
		logger.debug( "host: {} clusterFilter {} ", host, clusterFilter ) ;
				...
				.forEachRemaining( serviceInstanceName -> {
					...
					if ( active_services_all_hosts.get( serviceWithRuntimeStats.getName( ) ) == null ) {
						...
						if ( serviceWithConfiguration != null
							...
							ObjectNode clusterReport = null ;

							if ( serviceWithConfiguration.hasProcessGroup( ) ) {
							...
							logger.debug( "{} : numberRunning: {}", serviceWithRuntimeStats.toSummaryString( ),
									numberRunning ) ;

//							if ( serviceInstanceName.contains( CsapConstants.AGENT_NAME ) ) {
//
//								logger.info( "{} : numberRunning: {}", serviceWithRuntimeStats.toSummaryString( ), numberRunning ) ;
//
//							}

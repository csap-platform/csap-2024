...
import org.csap.agent.ui.rest.file.CsapStats ;
...
public class ApplicationConfiguration implements WebMvcConfigurer {
	...
	private int maxMsToListFiles = 500 ;
	...
	private String debugServiceName = "" ;


	private List<CustomProperty> customProperties = new ArrayList<>();
	

	private DbPerformanceCollector dbPerformanceCollector = new DbPerformanceCollector();
	...
	private boolean disableJmxCollection = false ;


	private boolean disableRemoteCollection = false ;

	private String zingPsPath = "/usr/bin/zing-ps" ;
	private String chefCookbookPath = "" ;
	...
	public int getMaxMsToListFiles( ) {
		return maxMsToListFiles;
	}

	public void setMaxMsToListFiles( int maxMsToListFiles ) {
		this.maxMsToListFiles = maxMsToListFiles;
	}

	public String getDebugServiceName( ) {
		return debugServiceName;
	}

	public void setDebugServiceName( String debugServiceName ) {
		this.debugServiceName = debugServiceName;
	}
	...
	public static class MyCustomSecurityPolicy implements CsapSecurityConfiguration.CustomHttpSecurity {
		...
		public void configure(
			...
			LoggerFactory
					...
					.debug( CsapApplication.arrowMessage( "Wiring in Custom Security rules" ) );
			...
			httpSecurity
					...
					.csrf( csrf -> csrf
							.requireCsrfProtectionMatcher( CsapSecurityConfiguration.buildRequestMatcher(
									"/login*",
									CsapConstants.FILE_URL + CsapFileManager.SAVE_URL,
					...
					.authorizeHttpRequests( ( requests ) -> requests
							...
							.requestMatchers(
									...
									"/swagger-resources/**",
									"/v2/**",
									CsapConstants.API_URL + "/**"
							).permitAll( )

							.requestMatchers(HttpMethod.GET,
									CsapConstants.FILE_URL + CsapFileMonitor.FILE_REMOTE_MONITOR,
									CsapConstants.FILE_URL + CsapStats.STATS_URL,
							...
							// BUILDS
							.requestMatchers(
									CsapConstants.SERVICE_URL + ServiceRequests.REBUILD_URL
							...
							//
							.requestMatchers(
									CsapConstants.SERVICE_URL + "/reImage",
							...
							.requestMatchers(HttpMethod.POST,
									OsExplorer.EXPLORER_URL + "/**",
									CsapConstants.ENCODE_FULL_URL,
									CsapConstants.NOTIFY_FULL_URL
							).access( ( authenticationSupplier, authorizationContext ) -> {
								return csapSecuritySettings.getRoles( )
										.check( authenticationSupplier, authorizationContext,
												CsapSecurityRoles.ADMIN_ROLE,
												CsapSecurityRoles.INFRA_ROLE
										);
							} )
							...
							.requestMatchers(
									...
									CsapConstants.FILE_URL + CsapFileManager.EDIT_URL,
									CsapConstants.FILE_URL + CsapFileManager.SAVE_URL,
									CsapConstants.FILE_URL + CsapFileManager.FILE_SYSTEM_URL,
...
/*
	public static class MyCustomSecurityPolicy_old implements CsapSecurityConfiguration.CustomHttpSecurity {
...
*/
	...
	public boolean isDisableJmxCollection ( ) {
	
		return disableJmxCollection ;
	
	}

	public void setDisableJmxCollection ( boolean disableJmxCollection ) {
	
		this.disableJmxCollection = disableJmxCollection ;
	
	}


	public String getChefCookbookPath( ) {
		return chefCookbookPath;
	}

	public void setChefCookbookPath( String chefCookbookPath ) {
		this.chefCookbookPath = chefCookbookPath;
	}



	public String getZingPsPath( ) {
		return zingPsPath;
	}

	File zingPsFile = null ;
	public File getZingPsFile( ) {

		if ( zingPsFile == null && StringUtils.isNotEmpty( getZingPsPath() ) ) {
			zingPsFile = new File (zingPsPath) ;
		}

		return zingPsFile;
	}

	public void setZingPsPath( String zingPsPath ) {
		this.zingPsPath = zingPsPath;
	}


	public boolean isDisableRemoteCollection( ) {
		return disableRemoteCollection;
	}

	public void setDisableRemoteCollection( boolean disableRemoteCollection ) {
		this.disableRemoteCollection = disableRemoteCollection;
	}
}

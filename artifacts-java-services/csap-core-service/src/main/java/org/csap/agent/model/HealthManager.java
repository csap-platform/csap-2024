...
public class HealthManager {
    ...
    public ArrayNode buildServiceAlertReport(
            String project,
            String filter
    ) {
    ...
    public ObjectNode build_health_report(
        ...
        if ( csapApis.application( ).isAdminProfile( ) ) {
            ...
            if ( csapApis.application( ).isMacOsProfileActive() ) {
                ...
                var agentHealthReport = build_health_report_for_agent( alertLevel, includeKubernetesWarnings,
                        null );
                var errors = ( ArrayNode ) agentHealthReport.get( HEALTH_SUMMARY );
        ...
        csapApis.application( )
                ...
                .filter( serviceInstance -> {
                    ...
                    if ( csapApis.application( ).isAgentProfile( ) ) {
                        ...
                        return serviceHealthCollected.has( serviceInstance.getName( ) );
...
//					if ( ! serviceHealthCollected.has( serviceInstance.getServiceName_Port( ) ) ) {
                    if ( !serviceHealthCollected.has( serviceInstance.getName( ) ) ) {

                        String message = hostName + ": " + serviceInstance.getName( )
                                + " No status found";
                        ...
                        try {
                            ...
                            List<String> serviceAlerts = ServiceAlertsEnum.getServiceAlertsAndUpdateCpu(
                                    ...
                                    includeKubernetesWarnings,
                                    serviceHealthCollected.get( serviceInstance.getName( ) ),
     ...
     * @param message
     */
    private void addNagiosStateMessage(
...
//				.rootProjectEnvSettings( ) ) ;
    ...
    private int suppressedAutoKills = 0;
    ...
    private void killServiceRunaway( ServiceInstance serviceInstance ) {
        ...
        if ( !serviceInstance.isAutoKillEnabled( ) ) {

            if ( suppressedAutoKills++ % 10 == 0 ) {

                logger.warn( "{} Suppressing auto kills for {}", serviceInstance.getName( ), reasons.toString( ) );
    ...
    public ObjectNode statusForAdminOrAgent(
        ...
        var secondary = csapApis.application( ).rootProjectEnvSettings( ).getLoadbalancerUrlAlt( );
        if ( StringUtils.isNotEmpty( secondary ) ) {
            hostSection.put( EnvironmentSettings.LOADBALANCER_URL + "-alt",
                    csapApis.application( ).rootProjectEnvSettings( ).getLoadbalancerUrlAlt( ) );
        }
    ...
    public ObjectNode buildCollectionSummaryReport(
            Map<String, ObjectNode> hostToAgentReport,
            String packageName
    ) {
    ...
    public ObjectNode build_host_status_using_cached_data(
        ...
        try {
            ...
            hostStatus.put( "chefCookBookCount", csapApis.osManager( )
                    .getCachedChefCookbookCount( ) );
    ...
    public ArrayNode buildServiceRuntimes( ServiceInstance definitionService ) {
        ...
        var serviceRuntimes = jsonMapper.createArrayNode( );
        ...
        if ( csapApis.application( ).isAgentProfile( ) ) {
            ...
            var hostAgentStatus = csapApis.application( )
            ...
            if ( hostAgentStatus != null ) {
                ...
                var collectedServiceStatus = csapApis.application( )
                        .getHostStatusManager( )
                        .getServiceRuntime(
                                definitionService.getHostName( ),
                                definitionService.getName( ) );
//				definitionService.getServiceName_Port( ) ) ;

//				logger.info( "collectedServiceStatus: {}", CSAP.jsonPrint( collectedServiceStatus ) ) ;
        ...
        for ( ContainerState container : runtimeService.getContainerStatusList( ) ) {
            ...
            if ( !inactiveKubernetesWorkerService ) {
                ...
                var uiInstanceData = runtimeService.build_ui_service_instance( container, containerIndex++ );
    ...
    private void add_definition_values_used_by_ui(
        ...
        uiInstanceData.put( "launchCred", definition.getLaunchCred( ) );

...
public class EnvironmentSettings {
    ...
    String applicationName = "DefaultName";
    List<String> subProjects = new ArrayList<>( );

    Map<String, String> fileManagerShortcuts = new LinkedHashMap<>( );
    private List<CustomProperty> customProperties = new ArrayList<>( );

    private Map<String, String> helpMenuUrlMap = new LinkedHashMap<>( );
    ...
    private ObjectNode hostSettings = jacksonMapper.createObjectNode( );
    private Map<String, String> hostCommands = new LinkedHashMap<>( );
    ...
    // Agent settings
    //
    private String buildCommand = "--exclude-task test"; // "-Dmaven.test.skip=true clean package" ;
    ...
    private boolean collectPssMemory = false;
    ...
    ObjectNode globalJobs = jacksonMapper.createObjectNode( );

    //
    // Collection settings
    //

    private boolean includeAltJavaAttributes = true;
    ...
    private String loadbalancerUrlAlt = "";
    ...
    public void loadSettings(
        ...
        processGlobalJobs( definition );

    }

    private void processGlobalJobs( JsonNode environmentSettings ) {

        var globalJobs = environmentSettings.path( "global-jobs" );

        if ( globalJobs.isObject( ) ) {

            this.globalJobs.setAll( ( ObjectNode ) globalJobs );

        } else {

            logger.debug( "No global jobs found" );

        }

        logger.debug( "globalJobs: {}", this.globalJobs );
    ...
    private void processProjectSettings( ) {
        ...
        if ( applicationSettings.isObject( ) ) {
            ...
            var subProjectDefinition = applicationSettings.path( ProjectLoader.SUB_PROJECTS );
            ...
            var fileManagerShortDef = applicationSettings.path( "file-manager-shortcuts" );

            if ( fileManagerShortDef.isArray( ) ) {

                getFileManagerShortcuts( ).clear( );
                CSAP.jsonStream( fileManagerShortDef )
                        .filter( JsonNode::isObject )
                        .map( shortcut -> ( ObjectNode ) shortcut )
                        .filter( shortcut -> StringUtils.isNotEmpty( shortcut.path( "label" ).asText( ) ) )
                        .filter( shortcut -> StringUtils.isNotEmpty( shortcut.path( "value" ).asText( ) ) )

                        .forEach( shortcut -> {

                            getFileManagerShortcuts( ).put(
                                    shortcut.path( "label" ).asText( ),
                                    shortcut.path( "value" ).asText( ) );
            ...
            logger.debug( "file manager shortcuts: {}", getFileManagerShortcuts( ) );

            loadCustomProperties( applicationSettings, subProjectDefinition );

            var helpItems = applicationSettings.path( "help-menu" );
            ...
            var launches = applicationSettings.path( "quick-launches" );
            ...
            var hostSettingsDef = applicationSettings.path( "host-settings" );

            if ( hostSettingsDef.isObject( ) ) {

                hostSettings = ( ObjectNode ) hostSettingsDef;

                var hostCommandReport = hostSettingsDef.path( "commands" );

                if ( hostCommandReport.isObject( ) ) {

                    try {

                        hostCommands = jacksonMapper.convertValue( hostCommandReport, Map.class );

                    } catch ( Exception e ) {

                        logger.warn( "Failed to process mappings" );
            ...
            var parsers = applicationSettings.path( "log-parsers" );
            ...
            var replacements = applicationSettings.path( "k8s-yaml-replacements" );
    ...
    private void loadCustomProperties(
            JsonNode applicationSettings,
            JsonNode subProjectDefinition
    ) {

        var customPropertiesDef = applicationSettings.path( "custom-properties" );

        if ( customPropertiesDef.isArray( ) ) {

            //
            // Add properties loaded from spring boot file, only in base env
            //
            if ( getCustomProperties( ).size( ) == 0 ) {

                getCustomProperties( ).addAll( csapApplication.configuration( ).getCustomProperties( ) );

            }

            //
            // add properties from definition
            //
            CSAP.jsonStream( customPropertiesDef )
                    .filter( JsonNode::isObject )
                    .map( customProp -> ( ObjectNode ) customProp )
                    .filter( customProp -> StringUtils.isNotEmpty( customProp.path( "name" ).asText( ) ) )
                    .forEach( customProp -> {

                        var customProperty = new CustomProperty( );
                        customProperty.setName( customProp.path( "name" ).asText( ) );
                        customProperty.setValue( customProp.path( "value" ).asText( ) );
                        customProperty.setKey( customProp.path( "key" ).asText( ) );
                        customProperty.setFile( customProp.path( "file" ).asText( ) );

                        var propIter = getCustomProperties( ).iterator( );

                        while ( propIter.hasNext( ) ) {

                            var currentItem = propIter.next( );

                            if ( currentItem.getName( ).equals( customProperty.getName( ) ) ) {
...
//			var lastFileName = "" ;
            ...
            for ( var customProperty : getCustomProperties( ) ) {

                if ( StringUtils.isNotEmpty( customProperty.getFile( ) ) ) {

                    var customFilePath = customProperty.getFile( );

                    // resolve any variables
                    for ( var customSubProperty : customProperties ) {

                        if ( StringUtils.isNotEmpty( customSubProperty.getValue( ) ) ) {

                            customFilePath = customFilePath.replaceAll(
                                    Matcher.quoteReplacement( customSubProperty.getName( ) ),
                                    customSubProperty.getValue( ) );

                        }

                    }

                    customProperty.setFile( customFilePath );

                }

            }

            logger.debug( getFormattedDefinitionProperties() );

        }

    }

    public String getFormattedDefinitionProperties() {

        return getCustomProperties( ).stream()
                .map( CustomProperty::toString )
                .collect( Collectors.joining("\n") ) ;
    }

    public String getFormattedFileManagerShortcuts() {

        return getFileManagerShortcuts( ).entrySet().stream()
                .map( shortcutEntry -> {
                    return "\t\t" + CSAP.pad(shortcutEntry.getKey() + ":" )  + shortcutEntry.getValue();
                } )
                .collect( Collectors.joining("\n") ) ;
    }
    public synchronized ArrayNode getQuickLaunchers( ) {
    ...
    private void processCoreSettings( JsonNode environmentDefinition )
        ...
        if ( environmentDefinition.has( "buildCommand" ) ) {

            this.buildCommand = environmentDefinition.path(
                    "buildCommand" ).asText( );
        ...
        if ( environmentDefinition.has( LOADBALANCER_URL + "-alt" ) ) {

            loadbalancerUrlAlt = trimSpacesAndVariables( environmentDefinition.path( LOADBALANCER_URL + "-alt" ).asText( ) );
    ...
    private void processAgentSettings( JsonNode lifecycleSettings )
        ...
        if ( agentSettings.isObject( ) ) {
            ...
            setCollectPssMemory( agentSettings.path( "collectPssMemory" ).asBoolean( collectPssMemory ) );
            ...
            maxJmxCollectionMs = agentSettings.path( "maxJmxCollectionMs" ).asLong( maxJmxCollectionMs );
            includeAltJavaAttributes = agentSettings.path( "includeAltJavaAttributes" ).asBoolean(
                    includeAltJavaAttributes );
    ...
    public void addHostLimits(
            String hostName,
            ObjectNode monitorDefaults
    ) {
    ...
    public boolean is_disk_monitored(
            String host,
            String mountPoint
    ) {
    ...
    //
    public long getMonitorForCluster(
            String clusterName,
    ...
    //
    public static long convertUnitToKb(
            String alertSetting,
            ServiceAlertsEnum alert
    ) {
    ...
    // searchText=appId=demo-xxxCsap,project=CSAP
    ...
    public String getAnalyticsUiUrl( ) {
        ...
        // special hook for when not using loadbalancers and publishing direct to event
        // endpoint
        // assumes events-service is deployed on same host as csap-admin
        resolvedAnalyticsUrl = resolvedAnalyticsUrl.replace(
                Matcher.quoteReplacement( ":7021" ),
                Matcher.quoteReplacement( ":8021" ) );
    ...
    public String toString( ) {
        ...
        return WordUtils.wrap( "EnvironmentSettings [helpUrBase=" + helpUrBase + ", limitSamples=" + limitSamples
                        ...
                        + ", historyUiUrl="
                        + historyUiUrl + ", mavenCommand=" + buildCommand + ", agentUser=" + agentUser + ", agentPass="
                        + agentPass
    ...
    public Map<String, String> getFileManagerShortcuts( ) {
    ...
    public ObjectNode getHostSettings( ) {

        return hostSettings;

    }

    Map<String, String> hostResolvedCommands;

    synchronized public Map<String, String> getHostCommands( ServiceInstance service ) {

        if ( hostResolvedCommands == null ) {

            hostResolvedCommands = new LinkedHashMap<>( );
            ;

            for ( var item : hostCommands.entrySet( ) ) {

                hostResolvedCommands.put(
                        service.resolveRuntimeVariables( item.getKey( ) ),
                        service.resolveRuntimeVariables( item.getValue( ) ) );

            }

        }

        return hostResolvedCommands;

    }

    public boolean isIncludeAltJavaAttributes( ) {

        return includeAltJavaAttributes;

    }

    public void setIncludeAltJavaAttributes( boolean javaCollectCoreOnly ) {

        this.includeAltJavaAttributes = javaCollectCoreOnly;

    }

    public ObjectNode getGlobalJobs( ) {

        return globalJobs;

    }

    public boolean isCollectPssMemory( ) {

        return collectPssMemory;

    }

    public void setCollectPssMemory( boolean collectPssMemory ) {

        this.collectPssMemory = collectPssMemory;

    }

    public String getLoadbalancerUrlAlt( ) {
        return loadbalancerUrlAlt;
    }

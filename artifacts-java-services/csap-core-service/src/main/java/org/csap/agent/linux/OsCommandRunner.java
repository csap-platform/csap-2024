...
public class OsCommandRunner {
	...
	public static String runCancellable (
		...
		theLogger.info( CSAP.buildDescription( "Running Script", 
				"agentRunUser",agentRunUser,
				"chownUserid",chownUserid,
				"timeOutSeconds", timeOutSeconds,
				"script", targetFile));
		//"{} max time {} seconds, Running: {} ", chownUserid, targetFile, timeOutSeconds ) ;
	...
	private File cancelFile = null ;
	 ...
	 void setCancelFile ( File cancelFile ) {
...
//				process.destroy( ) ;
	...
	public String executeString (
		...
		var summary = params.get( 0 ) ;
		var commandName = params.get( 0 ) ;
		// needs full path
		var workingDir = workingDirPossible.getAbsoluteFile() ;
		if (workingDir.getName().equals( "." ) ) {
			// Strip off for working context
			workingDir = workingDir.getParentFile() ;
		} else if (workingDir.getName().equals( ".." ) ) {
			// Strip off for working context
		...
		// change.
		var results = new StringBuffer( "Executing OS command on host "

		...
		for ( var containerIndex = 0; containerIndex < containerCount; containerIndex++ ) {
			...
			for ( OsProcessEnum os : OsProcessEnum.values( ) ) {
				...
				if ( isAggregateContainers && containerIndex > 0 ) {
					...
					var current = serviceMetricCollection.remove( 0 ).asLong( ) ;

					if ( metric.equals( OsProcessEnum.RSS_MEMORY )  ) {

						serviceMetricCollection.insert( 0, current + metricCollected.asLong( ) / 1024 ) ;

					} else if ( metric.equals( OsProcessEnum.PSS_MEMORY )  ) {

						serviceMetricCollection.insert( 0, current + metricCollected.asLong( ) / 1024) ;

					} else {

						serviceMetricCollection.insert( 0, current + metricCollected.asLong( ) ) ;
					...
					if ( metric.equals( OsProcessEnum.RSS_MEMORY ) ) {
						...
						serviceMetricCollection.insert( 0, metricCollected.asLong( ) / 1024 ) ;
						...
						serviceMetricCollection.insert( 0, metricCollected.asLong( ) ) ;
		...
		for ( var serviceId : serviceIds ) {
			...
			if ( serviceOsMetrics != null ) {
				...
				serviceSummaryReport.put( "numberOfSamples",
						numSamples + serviceSummaryReport.path( "numberOfSamples" ).asLong( 0 ) ) ;

				// add container counts for calculating total - unless - it is in aggregation
				// mode
				serviceSummaryReport.put( "countCsapMean",
						1 + serviceSummaryReport.path( "countCsapMean" ).asLong( 0 ) ) ;
				...
				for ( OsProcessEnum os : OsProcessEnum.values( ) ) {
					...
					if ( requestedSampleSize == -1 ) {
						...
						for ( int i = 0; i < requestedSampleSize
							...
							metricTotal += cacheArray.path( i ).asLong( ) ;

						}

						serviceSummaryReport.put( metric, metricTotal + serviceSummaryReport.path( metric ).asLong(
								0 ) ) ;

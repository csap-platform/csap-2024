...
public class OsCommands {
	...
	private List<String> pssMemory = NOT_INITIALIZED ;
	...
	public List<String> getProcessStatus ( ) {
		...
		if ( CsapApis.getInstance().application().isMacOsProfileActive()  ) {
			// https://ss64.com/osx/ps.html, https://ss64.com/osx/ps_keywords.html
			return List.of(
					"bash",
					"-c",
					"ps -e -o pcpu,rss,vsz,wq,ruser,pid,nice,ppid,args | grep -v -e RUSER -e '--login' -e '/usr/bin/login'"
			) ;
		}
	...
	public List<String> getPssMemory ( ) {

		return pssMemory ;

	}

	public void setPssMemory ( String pssMemory ) {

		this.pssMemory = toList( pssMemory ) ;
	...
	public List<String> getSystemProcessMetrics ( int seconds ) {
		...
		var sourceWithVariables = systemProcessMetrics ;
		if ( CsapApis.getInstance().application().isMacOsProfileActive()  ) {
			// https://ss64.com/osx/top.html
			sourceWithVariables = List.of(
					"bash",
					"-c",
					"top -l 0 -s $seconds"
			) ;

...
public class ProjectMigrator {
	...
	public ProjectMigrator ( boolean testMode ) {
		...
		if ( testMode ) {
			...
			logger.info( CsapApplication.smallTestHeader( "testMode: {}" ), testMode ) ;
	...
	public void migrateIfRequired ( File projectFile , ObjectNode projectDefinition ) {
		...
		if ( modelVersion < CURRENT_VERSION ) {
			...
			logger.warn( CsapApplication.header( "Strict: {} Project: {} version: {}, required is: {}" ),
					CsapApis.getInstance( ).application( ).configuration( ).isDefinitionStrictMode( ),
					projectFile.getName( ),
					modelVersion, CURRENT_VERSION ) ;

		}

		if ( ! CsapApis.getInstance( ).application( ).configuration( ).isDefinitionStrictMode( ) ) {
	...
	private void migrate_2_0_environments ( StringBuilder info , String fileName , ObjectNode modelDefinition ) {
		...
		if ( ( legacyCapability != null ) && legacyCapability.isObject( ) ) {
			...
			helpMenu.put( "User Guide", "https://somecompany2.atlassian.net/wiki/spaces/CSAP" ) ;
			helpMenu.put( "Release Notes",
					"https://somecompany2.atlassian.net/wiki/spaces/CSAP/pages/258211856/Release+Notes" ) ;
			helpMenu.put( "Quick Install",
					"https://somecompany2.atlassian.net/wiki/spaces/CSAP/pages/395282580/CSAP+Quick+Install" ) ;
			helpMenu.put( "Health Status",
					"https://somecompany2.atlassian.net/wiki/spaces/CSAP/pages/258211861/CSAP+Health" ) ;
			helpMenu.put( "Application Editor",
					"https://somecompany2.atlassian.net/wiki/spaces/CSAP/pages/258211875/CSAP+Application+Editor" ) ;
			helpMenu.put( "FAQ", "https://somecompany2.atlassian.net/wiki/spaces/CSAP/pages/347177032/CSAP+FAQ" ) ;

...
public class ModelApi {
    ...
    final static Logger logger = LoggerFactory.getLogger( ModelApi.class );
...
//	}
    ...
    public JsonNode serviceNames(
            ...
            @RequestParam ( defaultValue = "false" ) boolean includeOnlyAutoStart,
            @RequestParam ( defaultValue = "true" ) boolean sortUsingAutoStart,
            @RequestParam ( defaultValue = "false" ) boolean reverse
    ) {

        return getIdOrNames( includeOnlyAutoStart, sortUsingAutoStart, false, project, cluster, appendInfo, reverse );
    ...
    public JsonNode serviceNamesAll(
            ...
            @RequestParam ( required = false ) List<String> cluster,
            @RequestParam ( defaultValue = "false" ) boolean appendInfo,
            @RequestParam ( defaultValue = "false" ) boolean sortUsingAutoStart,
            @RequestParam ( defaultValue = "false" ) boolean reverse
    ) {

        return getIdOrNames( false, sortUsingAutoStart, false, project, cluster, appendInfo, reverse );
    ...
    public JsonNode serviceIds(
        ...
        return getIdOrNames( true, sortUsingAutoStart, true, project, cluster, appendInfo, reverse );

    }

    private JsonNode getIdOrNames(
            boolean includeOnlyAutostart,
            boolean sortAutoStart,
            boolean isId,
            String projectName,
        ...
        var services = project.getServiceEntryStreamInCurrentLC( ).flatMap(
        ...
        if ( sortAutoStart ) {
            ...
            instances = instances.sorted( ServiceBase.csapAutoStartComparing( ServiceInstance::startOrder ) );
    ...
    @CsapDoc ( notes = {
            ...
            "serviceName=" + CsapConstants.AGENT_NAME
    } )
    ...
    @GetMapping ( "/hosts/currentLifecycle/name/{serviceName}" )
    public JsonNode hostsInCurrentEnvironmentForService(
            @PathVariable ( "serviceName" ) String serviceName
    ) {
        ...
        var hosts = application.getLifeCycleServicesByMatch( serviceName ).stream( )
                .map( ServiceInstance::getHostName )
                .map( shortName -> application.getHostUsingFqdn( shortName ) )
                .collect( Collectors.toList( ) );

        return jacksonMapper.convertValue( hosts, ArrayNode.class );

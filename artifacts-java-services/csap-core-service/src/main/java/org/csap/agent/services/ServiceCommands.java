...
public class ServiceCommands {
					...
					if ( instance == null ) {
						...
						if ( auditUserid != null && auditUserid.length( ) > 0 ) {
							...
							csapApis.events( ).publishUserEvent(
									...
									"start parameters: " + commandArguments + "\n\n"
											+ "deplyment options:" + params + "\n" ) ;
	...
	public ObjectNode deployRemoteRequests (
		...
		if ( ! csapApis.application( ).isAdminProfile( ) ) {
			...
			if ( isPresent( repoPass ) ) {

				deployParameters.set( "repoPass", repoPass ) ;
			...
			services.stream( ).forEach( service_port -> {
				...
				try {
					...
					if ( instance == null ) {
						...
						var rebuildVariables = new LinkedMultiValueMap<String, String>( ) ;
						...
						rebuildVariables.set( "repoPass", repoPass ) ;
						...
						if ( auditUserid != null && auditUserid.length( ) > 0 ) {
							...
							csapApis.events( ).publishUserEvent(
									CsapEvents.CSAP_USER_SERVICE_CATEGORY + "/" + instance.getName( ),
									auditUserid, "Deploy Request Received",
									"start parameters: " + commandArguments + "\n\n"
											+ "rebuildVariables:" + filteredVariables ) ;

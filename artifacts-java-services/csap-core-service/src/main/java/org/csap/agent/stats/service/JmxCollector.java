...
public class JmxCollector {
    ...
    public void collectServicesOnHost( ) {
        logger.debug( "JMX collection disabled: {}", csapApis.application( ).configuration( ).isDisableJmxCollection( ) );

        if ( csapApis.application( ).configuration( ).isDisableJmxCollection( ) ) {
            return;
        }
        ...
        var serviceToCollectStream = csapApis.application( )
                .getActiveProject( )
        ...
        while ( true ) {
            ...
            var failedToCollectInstances = serviceToCollectStream
    ...
    private boolean executeJmxCollection(
            ServiceInstance serviceInstance,
            boolean isIgnoreCollectionFailure
    ) {

        var serviceNamePort = serviceInstance.getServiceName_Port( );
        ...
        var applicationResults = new ServiceCollectionResults( serviceInstance,
        ...
        if ( !serviceInstance.getDefaultContainer( ).isRunning( )
            ...
            if ( Application.isRunningOnDesktop( ) ) {
                ...
                if ( logger.isDebugEnabled( )
                        || ( serviceCollector.isShowWarnings( )
                        && !csapApis.application( ).isDesktopProfileActiveOrSpringNull( ) ) ) {

                    logger.warn( "{} JmxConnection using {} For desktop testing",
                            serviceInstance.getName( ), jmxHost );

                }

            }

            var performanceConfiguration = serviceInstance.getPerformanceConfiguration( );
            if ( performanceConfiguration != null ) {
                var jmxConfiguration = performanceConfiguration.path( "jmxHostOverride" ).asText( );
                var overHost = serviceInstance.getPerformanceConfiguration( ).path( "config" ).path( "jmxHostOverride" ).asText( );
                if ( StringUtils.isNotEmpty( overHost ) ) {
                    jmxHost = overHost;
                    logger.warn( "{} Over host found - JmxConnection using {} For desktop testing",
                            serviceInstance.getName( ), jmxHost );
                }
            }
...
//            }
            ...
            var jmxServiceCollectionUrl = "service:jmx:rmi:///jndi/rmi://" + jmxHost + ":" + jmxPort + "/jmxrmi";
            ...
            try {
                ...
                if ( serviceInstance
                    ...
                    try {
                        ...
                        if ( csapApis.application( ).configuration( ) != null
                                && csapApis.application( ).configuration( ).isJmxAuthentication( ) ) {

                            credentials = new String[]{
                                    csapApis.application( ).configuration( ).getJmxUser( ),
                                    csapApis.application( ).configuration( ).getJmxPass( )
                            };
                        ...
                        connector = connectWithTimeout(
                                new JMXServiceURL( jmxServiceCollectionUrl ),
                ...
                logger.debug( "{} Failed to get jmx data for service: {}", serviceNamePort, CSAP.buildCsapStack( e ) );
    ...
    private JMXConnector connectWithTimeout(
        ...
        logger.debug( "xxx Attempting connection to: {} with timeout : {} {}", url, requestTimeout, requestUnit );

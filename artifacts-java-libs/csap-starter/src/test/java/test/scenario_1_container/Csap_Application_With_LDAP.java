...
class Csap_Application_With_LDAP {
	...
	void beforeEach ( ) {
		...
		Assumptions.assumeTrue( csapSecurityConfiguration != null ) ;
			...
			CsapSecurityConfiguration.CustomHttpSecurity mySecurity = ((httpSecurity, securitySettings) -> {
				...
				httpSecurity
						...
						.authorizeHttpRequests( ( requests ) -> requests
								...
								//
								.requestMatchers(
										HI_SECURED_URL, SEC_USER, "/someUrlNeedingAdmin1", "/anotherUrlNeedingAdmin"
...
//						.access( CsapSecurityRoles.hasAny( CsapSecurityRoles.Access.view ) ) ;
	...
	public void lookup_user_in_ldap ( )
		...
		assertThat( ldapUserDetails.getDn( ).toLowerCase() )
				.as( "ldap user dn" )
				.contains( securitySettings.getProvider( ).getSearchUser( ).toLowerCase() ) ;
	...
	public void verify_user_info ( )
		...
		assertThat( userReport.path( "dn" ).asText( ) ).contains( "uid=peter.nightingale,ou=People,dc=workday,dc=com" ) ;
	...
	public void http_get_secured_resource_using_login_cookies ( )
		...
		logger.info( CsapApplication.testHeader( "Logging in to server" ) ) ;
		...
		logger.info( CsapApplication.testHeader( "Hitting: {}" ), simpleUrl ) ;
	...
	private ResponseEntity<String> performLogin ( ) {
		...
		logger.info("url: {} user: {} pass: {}", simpleUrl, ldapSettings.getUser( ), ldapSettings.getPass( )) ;
		formParams.set(

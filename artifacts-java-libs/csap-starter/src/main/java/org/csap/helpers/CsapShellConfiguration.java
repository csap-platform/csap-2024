...
public class CsapShellConfiguration {
	...
	final public static String PARAM_TIMEOUT="timeoutSeconds" ;
	@Inject
	Environment env;
	...
	public void execute( ) {
		...
		var timeoutSeconds = env.getProperty( PARAM_TIMEOUT, Integer.class ) ;

		StringBuilder info = new StringBuilder( );
		info.append( CSAP.padLine( "api" ) + api );
		info.append( CSAP.padLine( "apiUrl" ) + apiUrl );
		info.append( CSAP.padLine( "api timeout" ) + timeoutSeconds );
		...
		try {
			...
			if ( isTextResponse && params.equals( "none" ) ) {
				...
				ResponseEntity<String> response = getRestTemplate( timeoutSeconds ).getForEntity( apiUrl,
				...
				textResult = getRestTemplate( timeoutSeconds ).postForObject( apiUrl, formVariables, String.class );
...
//				logger.info( "GET to api: " + api + " with lab: " + env.getProperty( "lab" ) + " script: " + isParseResponse ) ;
				apiResponseInJsonFormat = getRestTemplate( timeoutSeconds ).getForObject( apiUrl, JsonNode.class );
				...
				apiResponseInJsonFormat = getRestTemplate( timeoutSeconds ).postForObject( apiUrl, formVariables, JsonNode.class );
...
//
//		int timeout = env.getProperty( "timeOutSeconds", Integer.class )  ;
//

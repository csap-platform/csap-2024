...
public class CsapWebServerConfig {
    ...
    private Connector createSslConnector( ) {
        ...
        logger.info( " csapWebSettings {}", settings.toString( ) );

        sslHostConfigCertificate.setCertificateKeystoreFile( settings.getSsl( ).getKeystoreFile( ) );
        sslHostConfigCertificate.setCertificateKeystoreType( settings.getSsl( ).getKeystoreType( ) );

        var decoded = csapEncrypt.decodeIfPossible( settings.getSsl( ).getKeystorePassword( ), logger );

        sslHostConfigCertificate.setCertificateKeystorePassword( decoded );
        sslHostConfigCertificate.setCertificateKeyAlias( settings.getSsl( ).getKeyAlias( ) );
    ...
    private void buildServerReport( WebServerInitializedEvent event ) throws IOException, Exception {
        ...
        for ( var tomcatConnector : tomcat.getService( ).findConnectors( ) ) {
            ...
            if ( protocolHandler instanceof AbstractHttp11JsseProtocol ) {
                ...
                var sslHostConfigs = sslProtocol.findSslHostConfigs( );

                if ( sslHostConfigs != null && sslHostConfigs.length > 0 ) {

                    for ( var sslHostConfig : sslHostConfigs ) {
                        connectorReport.append( CSAP.padLine( "getSslProtocol" ) + sslHostConfig.getSslProtocol( ) );

                        for ( var cert : sslHostConfig.getCertificates( ) ) {
                            connectorReport.append( CSAP.padLine( "getKeyAlias" ) + cert.getCertificateKeyAlias( ) );
                            connectorReport.append( CSAP.padLine( "getKeystoreFile" ) + cert.getCertificateKeystoreFile( ) );
							connectorReport.append( CSAP.padLine( "getKeystoreType" ) + cert.getCertificateKeystoreType( ) );
							connectorReport.append( CSAP.padLine( "getKeystoreProvider" ) + cert.getCertificateKeystoreProvider() );

                            if ( logger.isDebugEnabled( ) ) {

                                connectorReport.append( CSAP.padLine( "getKeystorePass" ) + cert.getCertificateKeystorePassword( ) );

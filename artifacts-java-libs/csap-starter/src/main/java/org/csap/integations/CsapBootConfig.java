...
public class CsapBootConfig implements ApplicationListener<ContextRefreshedEvent> {
	...
	@Bean
	@ConditionalOnMissingBean( CsapSecuritySettings.class )
	public CsapSecuritySettings desktopSecurity() {
		
		var securitySettings = new CsapSecuritySettings() ;
		
		CsapSecurityRoles roles = new CsapSecurityRoles( );
//		roles.set
		securitySettings.setRoles( roles );

...
window.handleConnectionError = function ( command, errorThrown ) {
    ...
    console.warn(`handleConnectionError: ${ command }`, errorThrown) ;
    ...
    //launch it.
    let theAlertifyDialog = alertify.connAlertLegGlob( $messageInHtml.html() );
...
if ( !window.console ) {
    ...
    ( function () {
        let names = [ "log", "debug", "info", "warn", "error", "assert", "dir",
...
function csap_flot_helpers() {
    ...
    return {
        ...
        getUsCentralTime: function ( localTime ) {
            let usCentral = new Date( localTime );
     ...
     */
        calculateUsCentralDays: function ( selectedTime ) {
            ...
            // let days=Math.round((nowDate.getTime() - selectedDate.getTime()
            // )/24/60/60/1000) ;
...
// function calculateUsCentralDays( selectedTime ) {
// let nowDate=new Date(Date.now());
// let selectedUsOffset = 5 * 60 * 60 * 1000 ; // 5 hours in milliseconds
// let selectedDate = new Date ( selectedTime + selectedUsOffset ) ;
//
// let nowUtc = new Date(nowDate.getUTCFullYear(), nowDate.getUTCMonth(),

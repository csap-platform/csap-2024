...
function api_navigator() {
    ...
    jQuery.expr[ ':' ].icontains = function ( element, index, match ) {
        ...
        let elementText = jQuery( element ).text() ;

        console.debug(`element`, elementText, `index: ${ index}, match `, match) ;
        return elementText.toUpperCase().contains( match[ 3 ].toUpperCase() ) ;
    ...
    function applyFilter() {
        ...
        if ( filter.length > 0 ) {
            ...
            $( 'td div.api-path:icontains("' + filter + '")', $rows ).closest("tr").show();

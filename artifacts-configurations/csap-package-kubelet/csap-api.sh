...
function installation_settings() {
	...
	if [ "$calico_ip_method" == "auto" ] ; then
		...
		elif (( $(ip a | grep ens160: | wc -l) > 0 )) ; then
			# mac os usb wire connection for vmware fusion
			# print_line "calico auto discovery: ip a matched ens160:" ; 
			calico_ip_method='interface=ens160'
		else

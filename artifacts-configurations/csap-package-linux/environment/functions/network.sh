...
function copy_remote() {
  ...
  if [[ "$password" == "file:"* ]] ; then
    ...
    local tokenFile="${password:5}";
    print_two_columns "user" "$user";
    ...
    for targetHost in $hosts; do
      ...
      hostOutput=$(scp -i $tokenFile  -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $fileToCopy $user@$targetHost:$destination 2>&1)
...
function run_remote() {
    ...
    for targetHost in $hosts; do
      ...
      for command in "${commands[@]}"; do
        ...
        if [[ "$command" == *';'* ]] ; then
          print_two_columns "token mode" "multiple commands, invoking via script wrapper in $(pwd)"

          if test -f script-wrapper ; then rm -f script-wrapper ; fi ;
          append_file "#!/bin/bash" script-wrapper false;
          append_line "$command" ;
          copy_remote $user $password "$targetHost" script-wrapper script-wrapper
          ssh -i $tokenFile -o StrictHostKeyChecking=no $user@$targetHost "chmod 755 ./script-wrapper"
          ssh -i $tokenFile -o StrictHostKeyChecking=no $user@$targetHost "sudo ./script-wrapper"
          if test -f script-wrapper ; then rm -f script-wrapper ; fi ;

        else
          print_two_columns "token mode" "prepending sudo"
          ssh -i $tokenFile -o StrictHostKeyChecking=no $user@$targetHost "sudo $command"
        fi

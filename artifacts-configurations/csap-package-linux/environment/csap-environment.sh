...
if test -z "$CSAP_FOLDER"  ; then
	...
	csapHomeFolder=$(cat /etc/passwd | grep csap | cut -d: -f6) ;

	export CSAP_FOLDER="$csapHomeFolder/csap-platform" ;
	if ! test -d "$CSAP_FOLDER" ; then
	  if test -d "$scriptDir" ; then
	    # mac os desktop from ide
	    CSAP_FOLDER="$(dirname $scriptDir)"
	  fi
	fi
	
	if test -z "$ENV_FUNCTIONS"  ; then
...
if [ "$toolsServer" == "" ] ; then
	toolsServer="csap-dev01.csap.org"
fi ;
...
if [[ "$csapName" != "" ]] ; then
	...
	csapGroupResourceFolder="$csapDefinitionResources/$csapProcessGroup"

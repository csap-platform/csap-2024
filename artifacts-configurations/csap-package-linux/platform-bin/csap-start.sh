...
if [ "$csapName" == "$csapAgentName" ] ; then
	...
	if [ "$isHotDeploy" != "1" ]  &&  [ "$csapServer" ==  "SpringBoot" ]; then
		...
		if [ "$csapPids" != "" ] ; then
			print_section "Found existing pid(s): '$csapPids'"
			print_command "ps -o pcpu,pid,args"  "$(ps -o pcpu,pid,args -p $csapPids )"

			delay_with_message 10 "start will resume in 10 seconds"
		fi ;
	fi ; 
...
if ! test -f "$csapPackageFolder/$csapName.war" \
	&& ! test -f "$csapPackageFolder/$csapName.zip"  \
	&& ! test -f "$csapPackageFolder/$csapName.jar"  \
	&& ! test -f "$csapPackageFolder/$csapProcessGroup.jar"  ; then

	if  test -f $csapResourceFolder/csap-api.sh ; then

		print_section "generated artifact found: $csapResourceFolder/csap-api.sh" ;

	elif test -f $csapGroupResourceFolder/csap-api.sh ; then

		print_section "generated artifact found: $csapGroupResourceFolder/csap-api.sh" ;

	else
		print_section "Did not find $csapResourceFolder/csap-api.sh, verify deployment"
		exit ;
	fi ;
fi ;
...
function configure_java_options() {
	...
	JAVA_OPTS="$javaOptions $csapParams" ;
	print_command "java options variable JAVA_OPTS" "$JAVA_OPTS" ;
}

if [[ "$csapServer" == "SpringBoot" ]] || \
	[[ "$csapTomcat" == "true" ]] || \
	[ "$csapJmxPort" != "" ] ; then 

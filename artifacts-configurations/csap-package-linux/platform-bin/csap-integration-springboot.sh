...
function startBoot() {
	...
	if  [ "$isSkip" != "1" ]  ; then
		...
		/usr/bin/unzip -qq -o $( getJarName ) -d $jarExtractDir
	...
	if [ "$csapDockerTarget" == "true" ]  ; then
		...
		if [[ "$csapParams" =~ DcsapNoSecurity ]] ; then
			print_section "Overiding security settings and disabling security and 1 hour timeouts";
			args="$args --csap.security.enabled=false --server.tomcat.keep-alive-timeout=3600000" ;
			# https://docs.spring.io/spring-boot/docs/current/reference/html/application-properties.html#application-properties.server.server.tomcat.max-keep-alive-requests

		elif [[ "$csapParams" =~ DcsapMemorySecurity ]] ; then
			print_section "Overiding security and forcing provider to memory";
			args="$args --csap.security.provider.type=memory" ;
...
function getJarName() {
	...
	if test -f $csapPackageFolder/$csapName.jar ; then
		echo $csapPackageFolder/$csapName.jar
	else
		echo $csapPackageFolder/$csapProcessGroup.jar
	fi ;

}

function configureDeployVersion() {
	if test -f $( getJarName ).txt ; then
	  local bootVersion="$(date +"%h-%d__%I-%M")"
		local versionFromFile=$(grep -o '<version>.*<' $( getJarName ).txt  | cut -d ">" -f 2 | cut -d "<" -f 1)
		print_two_columns "versionFromFile" "'$versionFromFile'"
		if [ "$versionFromFile" != "" ] ; then
		  bootVersion="$versionFromFile";
		fi
    ...
    print_two_columns "version" "Skipping - using manifest"
	fi ;
